...
public partial class QuestionBankSubSiteMaster : System.Web.UI.MasterPage
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        //lblcolname.Text = da.GetFunctionv("select collname from collinfo where  college_code='" + collegecode + "'");
        ...
        if (!IsPostBack)
        {
            ...
            {
                img_stfphoto.ImageUrl = "~/Handler/staffphoto.ashx?staff_code=" + Convert.ToString(Session["Staff_Code"]).Trim();
                imgstdphoto.ImageUrl = "~/Handler/staffphoto.ashx?staff_code=" + Convert.ToString(Session["Staff_Code"]).Trim();
                string stfdescode = "";
                sql = "select desig_code from stafftrans where staff_code='" + Convert.ToString(Session["Staff_Code"]).Trim() + "' and latestrec=1";
                stfdescode = da.GetFunctionv(sql);

                if (!string.IsNullOrEmpty(stfdescode))
                {
                    string stfdesigname = "";
                    sql = "select dm.desig_name from desig_master dm where dm.desig_code='" + Convert.ToString(stfdescode).Trim() + "' and collegecode=" + Convert.ToString(Session["collegecode"]).Trim();
                    stfdesigname = da.GetFunctionv(sql);

                    string staffname = "";
                    sql = "select staff_name from staffmaster where staff_code='" + Convert.ToString(Session["staff_code"]).Trim() + "'";
                    staffname = da.GetFunctionv(sql);

                    string deptname = "";
                    sql = "select dt.dept_acronym from Department dt,stafftrans st where dt.Dept_code=st.dept_code and staff_code='" + Convert.ToString(Session["staff_code"]).Trim() + "' and latestrec=1";
                    deptname = da.GetFunctionv(sql);
            ...
            {
                string staffname = "";
                sql = "select full_name from usermaster where user_code='" + Convert.ToString(Session["UserCode"]).Trim() + "'";
                staffname = da.GetFunctionv(sql);
                lbslstaffname.Text = Convert.ToString(staffname);
            }
        }
        try
        {
            ...
            SelQ = "  select distinct HeaderName from Security_Rights_Details where Rights_Code in(select rights_code from security_user_right where " + grouporusercode + " ) and ModuleName='Question'";
            SelQ = SelQ + " select ModuleName ,HeaderName ,Rights_Code ,ReportId ,ReportName ,PageName ,HelpURL  from Security_Rights_Details where Rights_Code in (select rights_code from security_user_right where " + grouporusercode + " ) and ModuleName='Question'  order by HeaderPriority, PagePriority asc";
