...
public partial class ReportCardVIToVIII : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            if (!IsPostBack)
            {
                ...
                ddlreporttype.Items.Insert(0, new ListItem("Report Card  VI- VIII", "1"));
                ddlreporttype.Items.Add(new ListItem("Report Card  VI- VIII", "1"));
    ...
    protected void ddlreporttype_OnSelectedIndexChanged(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                if (Convert.ToInt32(ddlreporttype.SelectedItem.Value) == 3 || Convert.ToInt32(ddlreporttype.SelectedItem.Value) == 4 || Convert.ToInt32(ddlreporttype.SelectedItem.Value) == 5 || Convert.ToInt32(ddlreporttype.SelectedItem.Value) == 6 || Convert.ToInt32(ddlreporttype.SelectedItem.Value) == 7 || Convert.ToInt32(ddlreporttype.SelectedItem.Value) == 8 || Convert.ToInt32(ddlreporttype.SelectedItem.Value) == 9 || Convert.ToInt32(ddlreporttype.SelectedItem.Value) == 10 || Convert.ToInt32(ddlreporttype.SelectedItem.Value) == 11)
                {
                    //GetTest();
                    btngrade.Visible = false;
                    ...
                    // btngo.Attributes.Add("Style", "margin-left: -171px;");
                    btngo.Attributes.Add("Style", "background-color: silver;    border: 2px solid white;    color: Black;    font-family: Book Antiqua;    font-size: medium;    font-weight: bold;    height: 27px;    margin-left: -171px;    margin-top: 40px;    position: absolute;    width: 42px;");
                }
                else
                {
                    ...
                    for (int roll = 0; roll < studgradeds.Tables[0].Rows.Count; roll++)
                    {
                        ...
                        if (ds.Tables[0].Rows.Count > 0 && count4 > 0)
                        {
                            ...
                            if (partsds.Tables[0].Rows.Count > 0)
                            {
                                ...
                                string strqueryytable = "select distinct tv.TextCode,tv.TextVal,ag.Grade,ag.description,cd.Mark from activity_gd ag ,CoCurrActivitie_Det cd,textvaltable tv where ag.ActivityTextVal=cd.ActivityTextVal and tv.TextCode=ag.ActivityTextVal and tv.TextCode=cd.ActivityTextVal and  cd.ActivityTextVal in ('" + allactivity + "') and cd.Roll_No='" + Roll_No + "' and cd.Mark between frompoint and topoint  and cd.Degree_Code=ag.Degree_Code  and cd.term=ag.term  and cd.term='" + term + "' order by tv.TextCode ";
                ...
                if (studgradeds.Tables[0].Rows.Count > 0)
                {
                    for (int roll = 0; roll < studgradeds.Tables[0].Rows.Count; roll++)
                    {
                        ...
                        if (ds.Tables[0].Rows.Count > 0 && count4 > 0)
                        {
                            ...
                            //newpdftabpage2 = table1a.CreateTablePage(new Gios.Pdf.PdfArea(mydoc, 60, 250, 300, 100));
                            //mypdfpage.Add(newpdftabpage2);
