...
public partial class MarkMod_PerformanceReportCardICSE : System.Web.UI.Page
{
    ...
    private void setLabelText()
    {
        try
        {
            ...
            {
                lblBatch.Text = "Year";
                spPageHeading.InnerHtml = "Performance Report Card ICSE I To III";
                Page.Title = "Performance Report Card ICSE I To III";
            }
            else
            {
                lblBatch.Text = "Batch";
    ...
    private void AcademicPerformanceReport(List<string> lstAppNo, string rollNo)
    {
        try
        {
            ...
            if (lstAppNo.Count > 0)
            {
                ...
                if (dtStudentDetails.Rows.Count > 0)
                {
                    foreach (DataRow drStudent in dtStudentDetails.Rows)
                    {
                        ...
                        DataTable dtPreviousSemesterTest = new DataTable();
                        ...
                        if (dsStudMarks.Tables.Count > 1 && dsStudMarks.Tables[1].Rows.Count > 0)
                        {
                            ...
                            dsStudMarks.Tables[1].DefaultView.RowFilter = " Batch_Year='" + batchYear + "' and degree_code='" + degreeCode + "' and semester = '" + semester + "' and sections='" + section + "' and subject_type<>'others' and subject_type<>'other' and App_No='" + appNo + "'";  //< modified on jan 8 2018 by prabha
                            dsStudMarks.Tables[1].DefaultView.Sort = "subjectpriority,subType_no,subject_code";
                            ...
                            dtDistinctCommonTestName = dsStudMarks.Tables[1].DefaultView.ToTable(true, "criteria");  //dtPreviousSemesterTest

                            // modified on jan 8 2018 by prabha
                            dsStudMarks.Tables[1].DefaultView.RowFilter = " Batch_Year='" + batchYear + "' and degree_code='" + degreeCode + "' and semester < '" + semester + "' and sections='" + section + "'";
                            dsStudMarks.Tables[1].DefaultView.Sort = "Criteria_no";
                            dtPreviousSemesterTest = dsStudMarks.Tables[1].DefaultView.ToTable(true, "criteria", "Criteria_no", "Convert_Minmark", "Convert_Maxmark");
                        ...
                        if (dtCurrentStudentMarks.Rows.Count > 0)
                        {
                            ...
                            foreach (DataRow drSubjects in dtCurrentSemesterSubject.Rows)
                            {
                                ...
                                string displayMark2 = string.Empty;
                                foreach (DataRow dtTest in dtDistinctTestName.Rows)
                                {
                                    ...
                                    if (subjectRow == dtCurrentSemesterSubject.Rows.Count)
                                    {
                                        ...
                                        if (dicTestWiseTotal.ContainsKey(studTestNo.Trim()) && !isAbsent)
                                        {
                                        ...
                                        {
                                            //tblStudentMarks.Cell(subjectRow + 1, testColumn).SetContent(((!isAbsent) ? "--" : ""));
                                            //tblStudentMarks.Cell(subjectRow + 2, testColumn).SetContent(((!isAbsent) ? "--" : ""));
                                        }
                                        tblStudentMarks.Cell(subjectRow + 1, testColumn).SetFont(fontReportContentBold);
                                        tblStudentMarks.Cell(subjectRow + 2, testColumn).SetFont(fontReportContentBold);
                                        ...
                                        tblStudentMarks.Cell(subjectRow + 2, 0).ColSpan = totalColumns - 1;
                                        if (testColumn == dtDistinctTestName.Rows.Count - dtOtherSubjects.Rows.Count)
                                        {
                                            //double sum = dicSubjectWiseOverAllTotal.Sum(x => Math.Round((x.Value), 0, MidpointRounding.AwayFromZero));
                                            double sum = dicSubjectWiseOverAllTotal.Sum(x => x.Value);
                                            ...
                                            double average = Math.Round(sum / dicSubjectWiseOverAllTotal.Count, 2, MidpointRounding.AwayFromZero);
                                            //double average = sum / dicSubjectWiseOverAllTotal.Count;
                                            tblStudentMarks.Cell(subjectRow + 2, testColumn + 1).SetContent((isAbsent) ? "--" : (sum > 0) ? average.ToString() : "--");
                                            tblStudentMarks.Cell(subjectRow + 2, testColumn + 1).SetFont(fontReportContentBold);
                            ...
                            PdfTable pdftblRemarks = pdfDocumentReport.NewTable(fontReportFooter, 1, 1, 1); //modified
                            ...
                            pdftblRemarks.SetColumnsWidth(new int[] { 180 }); //, 18
                            //if (len > 68)
                            //{
                            ...
                            if (dtPreviousStudentMarks.Rows.Count > 0)
                            {
                                ...
                                List<int> limaxMarks = new List<int>();
                                foreach (DataRow row in dtPreviousStudentMarks.Rows)
                                {
                                    limaxMarks.Add(Convert.ToInt32(Convert.ToString(row["marks_obtained"]).Trim()));
                                }
                                ...
                                if (dtTerm.Rows.Count > 0)
                                {
                                    ...
                                    foreach (DataRow drTerm in dtTerm.Rows)
                                    {
                                        ...
                                        foreach (DataRow drSubject in dtPreviousSemesterSubject.Rows)
                                        {
                                            ...
                                            bool isPreviousAbsent = false;
                                            //foreach (DataRow dtTest in dtDistinctTestName.Rows)  //modified by prabha 
                                            foreach (DataRow dtTest in dtPreviousSemesterTest.Rows)
                                            {
                                                string subjectMaxMark = string.Empty;
                                                string subjectminMark = string.Empty;
                                                string studTestNo = Convert.ToString(dtTest["Criteria_no"]).Trim();
                                                ...
                                                tblStudentPerformanceAnalysis.Cell(termRow, 1).SetContent("100");//maximum result to be displayed
                                                ...
                                                if (testColumn == dtDistinctTestName.Rows.Count - dtOtherSubjects.Rows.Count)
                                                {
                                                    if (dicSubjectWiseTotal.ContainsKey(subjectTCode.Trim().ToLower()))
                                                    {
                                                        ...
                                                        subjectWiseTotal = Math.Round((dicSubjectWiseTotal[subjectTCode.Trim().ToLower()]), 2, MidpointRounding.AwayFromZero);
                                                        //subjectWiseTotal = dicSubjectWiseTotal[subjectTCode.Trim().ToLower()];
                                                        tblStudentPerformanceAnalysis.Cell(termRow, subjectCol).SetContent((isPreviousAbsent) ? "--" : subjectWiseTotal.ToString());
                                                    }
                                                    else
                                                        tblStudentPerformanceAnalysis.Cell(termRow, subjectCol).SetContent((isPreviousAbsent) ? "--" : "--");
            ...
            if (status == true)
            {
                ...
                if (appPath != "")
                {
                    ...
                    string szFile = "PerformanceReportCardICSE_I_To_III" + DateTime.Now.ToString("ddMMyyyy") + DateTime.Now.Ticks + ".pdf";  //modified
        ...
        {
            da.sendErrorMail(e, collegeCode, "Performance Analysis Report card I - III");
        }
    }
