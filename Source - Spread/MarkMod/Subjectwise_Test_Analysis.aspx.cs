...
public partial class Subjectwise_Test_Analysis : System.Web.UI.Page
{
    ...
    protected void btngo_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            drAvg = dtStudTest.NewRow();
            drAvg["S.No"] = "S.No";
            drAvg["Roll No"] = "Roll No";
            drAvg["Name of The Student"] = "Name of The Student";
            dtStudTest.Rows.Add(drAvg);
            if (ddl_collage.Items.Count > 0)
                collegecode = Convert.ToString(ddl_collage.SelectedValue).Trim();
            else
            {
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (Cbl_test.Items.Count > 0)
                    {
                        ...
                        int colcnt = dtStudTest.Columns.Count;
                        string test = "";
                        string subjectan = "";
                        int count = 0;
                        for (int i = 0; i < Cbl_test.Items.Count; i++)
                        {
                            if (Cbl_test.Items[i].Selected == true)
                            {
                                ...
                                dtStudTest.Rows[0][Convert.ToString(Cbl_test.Items[i].Text).Trim()] = Convert.ToString(Cbl_test.Items[i].Text).Trim();
                                ...
                                if (test == "")
                                {
                                    test = (Convert.ToString(Cbl_test.Items[i].Text).Trim());

                                }

                                else
                                {
                                    //  dtStudTest.Columns.Add(test);
                                    test = test + "-" + (Convert.ToString(Cbl_test.Items[i].Text).Trim());
                                    dtStudTest.Columns.Add(test);
                                    dtStudTest.Rows[0][test] = test;
                                }
                                if (subjectan == "")
                                {
                                    subjectan = (Convert.ToString(Cbl_test.Items[i].Text).Trim());

                                }
                                else
                                {
                                    //  dtStudTest.Columns.Add(subjectan);
                                    subjectan = (subjectan + "&" + (Convert.ToString(Cbl_test.Items[i].Text).Trim())) + "AVG";
                                    dtStudTest.Columns.Add(subjectan);
                                    dtStudTest.Rows[0][subjectan] = (subjectan);
                                }
                            ...
                            for (int tst = 0; tst < test.Length; tst++)
                            {
                                ...
                                if (dvtestMark.Count > 0)
                                {
                                    ...
                                    if ((tst + 1) % 2 == 0 && tst != 0)
                                    {
                                        ...
                                        drAvg = dtAvg.NewRow();
                                        string mark1 = Convert.ToString(dtStudTest.Rows[i][cnt - 2]).Trim();
                                        string mark2 = Convert.ToString(dtStudTest.Rows[i][cnt - 1]).Trim();
                                       ...
                                       // dtStudTest.Rows[i+1][cnt] = Convert.ToString(diff);
                                       
                                        //dtStudTest.Rows[i+1][cnt + 1] = Convert.ToString(Math.Round(avg, 2)).Trim();
    ...
    protected void RowHead(GridView gvStudTest)
    {
        for (int head = 0; head < 1; head++)
        {
            gvStudTest.Rows[head].BackColor = ColorTranslator.FromHtml("#0CA6CA");
            gvStudTest.Rows[head].Font.Bold = true;
            gvStudTest.Rows[head].HorizontalAlign = HorizontalAlign.Center;
