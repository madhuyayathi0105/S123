/*
  Code Started by Mohamed Idhris Sheik Dawood on 30/06/2016
*/
using System;
using System.Data;
...
public partial class RollNoGeneration : System.Web.UI.Page
{
    int collegeCode = 0;
    int userCode = 0;
    ...
    protected void btn_go_Click(object sender, EventArgs e)
    {
        ...
        catch (Exception ex)
        {
            spreadStudList.Visible = false;
            lbl_errormsg.Visible = true;
            lbl_errormsg.Text = "No Records Found"; DA.sendErrorMail(ex, collegeCode.ToString(), "RollNoAllocation.aspx");
        }
    }
    ...
    protected void btn_printmaster_Click(object sender, EventArgs e)
    {
        try
        {
            string degreedetails = "Roll Number Generation Report";
            string pagename = "RollNoGeneration.aspx";
    ...
    protected void btn_go1_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
            {
                ...
                spreadStudAdd.Sheets[0].Columns[3].Locked = false;
        ...
        catch (Exception ex)
        {
            spreadStudAdd.Visible = false;
            lbl_errormsg1.Visible = true;
            lbl_errormsg1.Text = "No Records Found"; DA.sendErrorMail(ex, Convert.ToString(ddl_college1.SelectedValue), "RollNoAllocation.aspx");
        }
    }
    ...
    protected void btn_confirmRes_yes_Change(object sender, EventArgs e)
    {
        ...
        try
        {
            for (int i = 1; i < spreadStudAdd.Rows.Count; i++)
            {
                ...
                string adm_no = Convert.ToString(spreadStudAdd.Sheets[0].Cells[i, 2].Text).Trim();
                spreadStudAdd.Sheets[0].Cells[i, 3].Text = adm_no;
                DA.update_method_wo_parameter("update registration set roll_no='" + adm_no + "' where app_no='" + app_no + "'", "Text");
            }
            spreadStudAdd.SaveChanges();
        }
        catch { }
    ...
    protected void btnGenerate_Click(object sender, EventArgs e)
    {
        ...
        if (InputCheck(out startNo, out size))
        {
            ...
            {
                string app_no = Convert.ToString(spreadStudAdd.Sheets[0].Cells[i, 0].Tag).Trim();
                string rollnoNew = generateRollNo(ref startNo, size, app_no);
                spreadStudAdd.Sheets[0].Cells[i, 3].Text = rollnoNew;
                startNo++;
            }
            spreadStudAdd.SaveChanges();
        }
    }
    private string generateRollNo(ref Int64 startNo, byte size, string app_no)
    {
        byte sizeConst = size;
        ...
        rollNoExist:
            ...
            rollNo = val + sbval.ToString();
            //Roll No verification for already existing
            string rollChk = DA.GetFunction("select roll_no from registration where roll_no='" + rollNo + "'  and app_no <> '" + app_no + "'").Trim();
            if (rollChk != "0")
            {
                startNo++;
                size = sizeConst;
    ...
    protected void btn_Save_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            spreadStudAdd.SaveChanges();
            //barath 09.02.18
            string AlreadyAvailableRollNo = string.Empty;
            for (int apI = 1; apI < spreadStudAdd.Sheets[0].Rows.Count; apI++)
            {
                ...
                if (checkval == 1)
                {
                    ...
                    string NewrollNo = Convert.ToString(spreadStudAdd.Sheets[0].Cells[apI, 3].Text);
                    string RollNoAvailable = DA.GetFunction("select Roll_No from registration where Roll_No='" + NewrollNo + "'");
                    if (string.IsNullOrEmpty(RollNoAvailable) || RollNoAvailable == "0")
                    {
                        string Oldrollno = Convert.ToString(DA.GetFunction("select Roll_No from registration where App_No='" + appNo + "'"));
                        Hashtable hat = new Hashtable();
                        hat.Add("@OldRollNo", Oldrollno);
                        hat.Add("@NewRollNo", NewrollNo);
                        DA.update_method_with_parameter("StudentRollNoUpdate", hat, "sp");
                        saved++;
                    }
                    else
                    {
                        AlreadyAvailableRollNo += RollNoAvailable + ",";
                    }
                }
            }
            //for (int apI = 1; apI < spreadStudAdd.Sheets[0].Rows.Count; apI++)
            //{
            //    string appNo = Convert.ToString(spreadStudAdd.Sheets[0].Cells[apI, 0].Tag);
            //    string rollNo = Convert.ToString(spreadStudAdd.Sheets[0].Cells[apI, 3].Text);
            //    string upQ = "update Registration set Roll_No='" + rollNo + "' where app_no='" + appNo + "'";
            //    int upOk = DA.update_method_wo_parameter(upQ, "Text");
            //    if (upOk > 0)
            //    {
            //        string upAttQ = "update Attendance set Roll_No='" + rollNo + "' where Roll_No='" + rollNo + "'";
            //        DA.update_method_wo_parameter(upAttQ, "Text");

            //        string upsCQ = "update subjectChooser set Roll_No='" + rollNo + "' where Roll_No='" + rollNo + "'";
            //        DA.update_method_wo_parameter(upsCQ, "Text");

            //        string upsCnQ = "update subjectChooser_New set Roll_No='" + rollNo + "' where Roll_No='" + rollNo + "'";
            //        DA.update_method_wo_parameter(upsCnQ, "Text");

            //        string upsCnsQ = "update subjectChooser_New_Spl set Roll_No='" + rollNo + "' where Roll_No='" + rollNo + "'";
            //        DA.update_method_wo_parameter(upsCnsQ, "Text");

            //        string upMeQ = "update mark_entry set Roll_No='" + rollNo + "' where Roll_No='" + rollNo + "'";
            //        DA.update_method_wo_parameter(upMeQ, "Text");

            //        string upResQ = "update Result set Roll_No='" + rollNo + "' where Roll_No='" + rollNo + "'";
            //        DA.update_method_wo_parameter(upResQ, "Text");

            //        string upEAQ = "update exam_application set Roll_No='" + rollNo + "' where Roll_No='" + rollNo + "'";
            //        DA.update_method_wo_parameter(upEAQ, "Text");

            //        string upODQ = "update Onduty_Stud set Roll_No='" + rollNo + "' where Roll_No='" + rollNo + "'";
            //        DA.update_method_wo_parameter(upODQ, "Text");

            //        string upsHrAQ = "update specialhr_attendance set Roll_No='" + rollNo + "' where Roll_No='" + rollNo + "'";
            //        DA.update_method_wo_parameter(upsHrAQ, "Text");
            ...
            //}
            imgAlert.Visible = true;
            lbl_alert.Text = String.Format("Saved : " + saved + ". \n\n Already Available : " + AlreadyAvailableRollNo.TrimEnd(','));
        }
        catch (Exception ex) { DA.sendErrorMail(ex, Convert.ToString(ddl_college1.SelectedValue), "RollNoAllocation.aspx"); }
        btn_go1_Click(sender, e);
    }
