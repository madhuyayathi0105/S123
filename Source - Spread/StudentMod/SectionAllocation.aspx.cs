/*
  Code Started by Mohamed Idhris Sheik Dawood on 29/06/2016
*/
using System;
using System.Data;
...
public partial class SectionAllocation : System.Web.UI.Page
{
    int collegeCode = 0;
    int userCode = 0;
    ...
    protected void btn_go_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (batch_year != string.Empty && degCode != string.Empty && branch != string.Empty && cusem != string.Empty)
            {
                ...
                selectquery = "select Roll_No,Roll_Admit,smart_serial_no,r.Stud_Name,d.Degree_Code,(C.Course_Name +' - '+ dt.Dept_Name) as Department,Reg_No,r.App_No,c.type,isnull(r.Sections,'') as Sections , case when a.sex=0 then 'Male' when a.sex=1 then 'Female' when a.sex=2 then 'Transgender' else 'N/A' end as Gender   from Registration r,Degree d,Department dt,Course c,applyn a where a.app_no=r.app_no and r.degree_code =d.Degree_Code and dt.Dept_Code =d.Dept_Code and c.Course_Id =d.Course_Id and CC=0 and DelFlag =0 and Exam_Flag <>'DEBAR' and r.Batch_Year =" + batch_year + " and r.degree_code in ('" + branch + "')  and isnull(r.Sections,'') in ('" + section + "')  and r.current_semester in('" + cusem + "')  and isnull(a.admission_status,'0')='1'  " + stream + " ";//order by Department,r.Sections asc
    ...
    public void bindsec1()
    {
        try
        {
            ...
            string strsql1 = "select distinct sections  from registration where  college_code='" + Convert.ToString(ddl_college1.SelectedValue) + "' and  sections is not null and ltrim(sections)<>'' and sections not in("+ secvar +") order by sections ";
            DataSet ds = DA.select_method_wo_parameter(strsql1, "text");
            if (ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
            {
                for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                {
                    ddl_sec1dest.Items.Add(ds.Tables[0].Rows[i]["sections"].ToString ());
                    ddl_sec1.Items.Add(ds.Tables[0].Rows[i]["sections"].ToString());
                }
            }
            //====================================================================
            ddl_sec1.Items.Add("");
    ...
    public void bindsec1Old()
    {
        try
        {
            ...
            if (dsSec.Tables.Count > 0 && dsSec.Tables[0].Rows.Count > 0)
            {
                ...
                ddl_sec1.DataBind();
                ddl_sec1dest.DataSource = dsSec;
                ddl_sec1dest.DataTextField = "sections";
                ddl_sec1dest.DataValueField = "sections";
                ddl_sec1dest.DataBind();
            }
            //if (ddl_sec1.Items.Count == 0)
            //{
            //    ddl_sec1.Items.Add(item);
            //}
            //if (ddl_sec1dest.Items.Count == 0)
            //{
            //    ddl_sec1dest.Items.Add(item);
            //}
        }
        catch (Exception ex) { }
    }
    ...
    protected void btn_go1_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (batch_year != string.Empty && degCode != string.Empty && branch != string.Empty && cusem != string.Empty)
            {
                ...
                selectquery = "select Roll_No,Roll_Admit,r.smart_serial_no,r.Stud_Name,d.Degree_Code,(C.Course_Name +' - '+ dt.Dept_Name) as Department,Reg_No,r.App_No,c.type,isnull(r.Sections,'') as Sections , case when a.sex=0 then 'Male' when a.sex=1 then 'Female' when a.sex=2 then 'Transgender' else 'N/A' end as Gender   from Registration r,Degree d,Department dt,Course c,applyn a where r.app_no=a.app_no and r.degree_code =d.Degree_Code and dt.Dept_Code =d.Dept_Code and c.Course_Id =d.Course_Id and CC=0 and DelFlag =0 and Exam_Flag <>'DEBAR' and r.Batch_Year =" + batch_year + " and r.degree_code in ('" + branch + "')  and isnull(r.Sections,'') in ('" + section + "') and r.current_semester in('" + cusem + "')  and isnull(a.admission_status,'0')='1'   " + stream + " ";//modified
            ...
            if (ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
            {
                ...
                spreadStudAdd.Sheets[0].ColumnHeader.Cells[0, 9].Text = "Gender";
                ...
                spreadStudAdd.Columns[9].Width = 60;
                FarPoint.Web.Spread.TextCellType txtRollno = new FarPoint.Web.Spread.TextCellType();
                FarPoint.Web.Spread.TextCellType txtRegno = new FarPoint.Web.Spread.TextCellType();
                FarPoint.Web.Spread.TextCellType txtRollAd = new FarPoint.Web.Spread.TextCellType();
                FarPoint.Web.Spread.TextCellType txtAppno = new FarPoint.Web.Spread.TextCellType();
                ...
                for (int row = 0; row < ds.Tables[0].Rows.Count; row++)
                {
                    ...
                    spreadStudAdd.Sheets[0].Cells[spreadStudAdd.Sheets[0].RowCount - 1, 9].Text = Convert.ToString(ds.Tables[0].Rows[row]["Gender"]);
                }
    ...
    protected void btn_Save_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (ddl_sec1dest.Items.Count > 0)
            {
                string desSec = ddl_sec1dest.SelectedItem.Text.Trim();
                string srcSec = ddl_sec1.SelectedItem.Text.Trim();
                string batchYr = ddl_batch1.Items.Count > 0 ? ddl_batch1.SelectedItem.Value : "0";
                string degreeCode = ddl_branch1.Items.Count > 0 ? ddl_branch1.SelectedItem.Value : "0";
                if (desSec != srcSec)
                {
                    if (true)
                    {
                        ...
                        if (checkedOK(out appNoList, out rollnoList))
                        {
                            for (int apI = 0; apI < appNoList.Count; apI++)
                            {
                                ...
                                string upQ = "update Registration set sections='" + desSec + "' where app_no='" + appNo + "'";
                                ...
                                if (upOk > 0)
                                {
                                    ...
                                    updateMarks(appNo, rollNo, degreeCode, batchYr, srcSec, desSec);
                                    saved++;
                                }
                                else
                                {
    ...
    private void updateMarks(string appNo, string rollNo, string degreeCode, string batchYr, string srcSec, string desSec)
    {
        try
        {
            string selSylMaster = "select syll_code from syllabus_master where degree_code ='" + degreeCode + "' and Batch_Year ='" + batchYr + "' ";
            DataSet dsSylMaster = new DataSet();
            dsSylMaster = DA.select_method_wo_parameter(selSylMaster, "Text");
            if (dsSylMaster.Tables.Count > 0 && dsSylMaster.Tables[0].Rows.Count > 0)
            {
                for (int sylCd = 0; sylCd < dsSylMaster.Tables[0].Rows.Count; sylCd++)
                {
                    string sylCode = Convert.ToString(dsSylMaster.Tables[0].Rows[sylCd][0]);
                    string selCriteria = "select criteria_no from CriteriaForInternal where syll_code ='" + sylCode + "'";
                    DataSet dsCriteria = new DataSet();
                    dsCriteria = DA.select_method_wo_parameter(selCriteria, "Text");
                    if (dsCriteria.Tables.Count > 0 && dsCriteria.Tables[0].Rows.Count > 0)
                    {
                        for (int cri = 0; cri < dsCriteria.Tables[0].Rows.Count; cri++)
                        {
                            string criCode = Convert.ToString(dsCriteria.Tables[0].Rows[cri][0]);
                            string selExtype = "select staff_code,isnull(subject_no,0) as subject_no,duration,convert(varchar(10),entry_date,101) as entry_date,convert(varchar(10),exam_date,101) as exam_date,isnull(batch_year,0) as    batch_year, isnull(max_mark,0) as  max_mark, isnull(min_mark,0) as min_mark, exam_code,sections, isnull(new_maxmark,0) as  new_maxmark, isnull(new_minmark,0) as new_minmark,isnull(start_period,0) as  start_period, isnull(end_period,0) as  end_period,isnull(islock,'0') as islock,convert(varchar(10),elockdate,101) as elockdate from Exam_type where  criteria_no='" + criCode + "' and sections ='" + srcSec + "'";
                            DataSet dsExtype = new DataSet();
                            dsExtype = DA.select_method_wo_parameter(selExtype, "Text");
                            if (dsExtype.Tables.Count > 0 && dsExtype.Tables[0].Rows.Count > 0)
                            {
                                for (int exT = 0; exT < dsExtype.Tables[0].Rows.Count; exT++)
                                {
                                    string staff_code = Convert.ToString(dsExtype.Tables[0].Rows[exT]["staff_code"]).Trim();
                                    string subject_no = Convert.ToString(dsExtype.Tables[0].Rows[exT]["subject_no"]).Trim();
                                    string duration = Convert.ToString(dsExtype.Tables[0].Rows[exT]["duration"]).Trim();
                                    string entry_date = Convert.ToString(dsExtype.Tables[0].Rows[exT]["entry_date"]).Trim();
                                    string exam_date = Convert.ToString(dsExtype.Tables[0].Rows[exT]["exam_date"]).Trim();
                                    string batch_year = Convert.ToString(dsExtype.Tables[0].Rows[exT]["batch_year"]).Trim();
                                    string max_mark = Convert.ToString(dsExtype.Tables[0].Rows[exT]["max_mark"]).Trim();
                                    string min_mark = Convert.ToString(dsExtype.Tables[0].Rows[exT]["min_mark"]).Trim();
                                    string exam_code = Convert.ToString(dsExtype.Tables[0].Rows[exT]["exam_code"]).Trim();
                                    string sections = Convert.ToString(dsExtype.Tables[0].Rows[exT]["sections"]).Trim();
                                    string new_maxmark = Convert.ToString(dsExtype.Tables[0].Rows[exT]["new_maxmark"]).Trim();
                                    string new_minmark = Convert.ToString(dsExtype.Tables[0].Rows[exT]["new_minmark"]).Trim();
                                    string start_period = Convert.ToString(dsExtype.Tables[0].Rows[exT]["start_period"]).Trim();
                                    string end_period = Convert.ToString(dsExtype.Tables[0].Rows[exT]["end_period"]).Trim();
                                    string islock = Convert.ToString(dsExtype.Tables[0].Rows[exT]["islock"]).Trim();
                                    if (islock.ToUpper() == "FALSE")
                                    {
                                        islock = "0";
                                    }
                                    else
                                        if (islock.ToUpper() == "TRUE")
                                        {
                                            islock = "1";
                                        }
                                    string elockdate = Convert.ToString(dsExtype.Tables[0].Rows[exT]["elockdate"]).Trim();
                                    //Insert for destination section if not exist
                                    string insQ = "if not exists(select exam_code from Exam_type where  criteria_no='" + criCode + "' and sections ='" + desSec + "' and subject_no='" + subject_no + "') Insert into Exam_Type(criteria_no,staff_code,subject_no,duration,entry_date,exam_date,batch_year,max_mark,min_mark,sections,new_maxmark,new_minmark,start_period,end_period,islock,elockdate) values (" + criCode + ",'" + staff_code + "'," + subject_no + ",'" + duration + "','" + entry_date + "','" + exam_date + "'," + batch_year + "," + max_mark + "," + min_mark + ",'" + desSec + "'," + new_maxmark + "," + new_minmark + "," + start_period + "," + end_period + ",'" + islock + "','" + elockdate + "')";
                                    DA.update_method_wo_parameter(insQ, "Text");
                                    string new_exam_code = DA.GetFunction("select exam_code from Exam_type where  criteria_no='" + criCode + "' and sections ='" + desSec + "' and subject_no='" + subject_no + "'").Trim();
                                    double marks_obtained = Convert.ToDouble(DA.GetFunction("select isnull(marks_obtained,0) from Result where exam_code =" + exam_code + " and roll_no ='" + rollNo + "'").Trim());
                                    if (new_exam_code != "0" && new_exam_code != "" && marks_obtained > 0)
                                    {
                                        string insUpResult = "if exists (select marks_obtained from Result where exam_code =" + new_exam_code + " and roll_no ='" + rollNo + "') update Result set marks_obtained=" + marks_obtained + " where  exam_code =" + new_exam_code + " and roll_no ='" + rollNo + "' else Insert into Result(marks_obtained,exam_code,roll_no) values(" + marks_obtained + "," + new_exam_code + ",'" + rollNo + "')";
                                        DA.update_method_wo_parameter(insUpResult, "Text");
                                    }
                                }
                            }
                        }
