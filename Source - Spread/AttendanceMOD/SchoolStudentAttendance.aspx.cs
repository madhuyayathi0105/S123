...
public partial class SchoolStudentAttendance : System.Web.UI.Page
{
    ...
    protected void grid_view_employee_RowCommadOld(object sender, GridViewCommandEventArgs e)
    {
        try
        {
            ...
            if (e.CommandName == "Select")
            {
                ...
                if (dsselect11.Tables[0].Rows.Count > 0)
                {
                    ...
                    if (halforfull == true)
                    {
                        ...
                        if (txtsec.Text != "---Select---")
                        {
                            ...
                            else
                            {
                                ...
                                if (dsselect1.Tables[0].Rows.Count > 0)
                                {
    ...
    protected void Button2_ClickOld(object sender, EventArgs e)
    {
        string sqlsec = string.Empty;
    ...
    public void SendingSms(string rollno, string appno, string regno, string admno, string date, string hour, string college, string course, string sectionall, string LeaveCodea)
    {
        try
        {
            ...
            if (check > 0)
            {
                ...
                if (ds1.Tables[0].Rows.Count > 0 && dsMobile.Tables[0].Rows.Count > 0)
                {
                    ...
                    string leavetype = Attmark(LeaveCodea);
                            ...
                            if (splittemplate.Length > 0)
                            {
                                for (int j = 0; j <= splittemplate.GetUpperBound(0); j++)
                                {
                                    if (splittemplate[j].ToString() != "")
                                    {
                                        ...
                                        {
                                            //aruna 12oct2017 MsgText = MsgText + " " + Hour + " hour Absent";
                                            //aruna 12oct2017
                                            MsgText = MsgText + " " + leavetype;//Hour + " hour  " +
                                        ...
                                        {
                                            string[] dtsplit = txtfrom.Text.Split('/');
                                            DateTime dtdate = Convert.ToDateTime(dtsplit[1] + "/" + dtsplit[0] + "/" + dtsplit[2]);
                                            MsgText = MsgText + " " + dtdate.ToString("dd/MM/yyyy");
                                        }
                    ...
                    {
                        MsgText = "Dear Parent, Good Morning. This Message from" + " " + collegename1 + ". Your ward " + dsMobile.Tables[0].Rows[0]["StudName"].ToString() + " of " + coursename1 + "-" + section + " is found " + leavetype + "  " + Hour + " hour. Thank you";
                    }
    ...
    public static string CheckUserName(string rollno, string batch, string degree, string sem, string sec, string entryby)
    {
        ...
        try
        {
            ...
            if (user_name.Trim() != "" && user_name != null)
            {
                ...
                {
                    string firstvalue = splitusername[i].Split('*')[0].ToString();
                    if (firstvalue.Trim() != "")
                    {
                        string query = dd.GetFunction("select roll_no from registration  where roll_admit ='" + firstvalue + "' and batch_year='" + batch + "' and degree_code='" + degree + "' and current_semester='" + sem + "' " + strsec + " and cc=0 and delflag=0 and exam_flag<>'debar'");
                        if (entryby == "1")
                        {
                            query = dd.GetFunction("select Reg_no from registration  where roll_no ='" + firstvalue + "' and batch_year='" + batch + "' and degree_code='" + degree + "' and current_semester='" + sem + "' " + strsec + " and cc=0 and delflag=0 and exam_flag<>'debar'");
                        }
                        if (entryby == "2")
                        {
    ...
    protected void attendanceMark(string appno, int mnthYear, string attDay, int noMaxHrsDay, int noFstHrsDay, int noSndHrsDay, int noMinFstHrsDay, int noMinSndHrsDay, string DateVal, string collegecode, string leavecode)
    {
        try
        {
            ...
            if (dsload.Tables.Count > 0 && dsload.Tables[0].Rows.Count > 0)
            {
                ...
                int matt = attendanceSet(MPCnt, MnullCnt, noMinFstHrsDay, leavecode, AttValueMrng);
                int eatt = attendanceSet(EPCnt, EnullCnt, noMinSndHrsDay, leavecode, AttvalueEve);
    ...
    protected int attendanceSet(int attCnt, int nullCnt, int hrCntCheck, string leave, Dictionary<int, int> val)
    {
        int attVal = 0;
        try
        {
    ...
    protected void grid_view_employee_RowCommad(object sender, GridViewCommandEventArgs e)
    {
        try
        {
            ...
            if (e.CommandName == "Select")
            {
                ...
                loadAttndPop((ddlcollege.Items.Count > 0 ? ddlcollege.SelectedValue : "13"), degree_code.Trim(), current_semester.Trim(), batch_Year.Trim(), sections.Trim(), confromdate);
            }
        }
        catch (Exception ex)
        {
    ...
    private void loadAttndPop(string collegeCode, string degCode, string curSem, string batch, string section, DateTime fromDate)
    {
        mdl_full_employee_details.Show();
        pnl_employee_details.Visible = true;
        divPopAttnd.Visible = true;
        ...
        string getshedulockva = d2.GetFunctionv("select value from Master_Settings where settings='Attendance Discount'");
        ...
        getshedulockva = d2.GetFunctionv("select value from Master_Settings where settings='Attendance Debar'");
        if (getshedulockva.Trim() == "1" || getshedulockva.Trim().ToLower() == "true")
        {
        ...
        //{
        //    strfeeofrollquery = "select r.Roll_No,r.Reg_No,r.Stud_Name,s.Purpose,s.hourse from Onduty_Stud s,Registration r where s.roll_no=r.Roll_No and r.Batch_Year='" + ddlbatch.SelectedValue.ToString() + "' and r.degree_code=" + ddlbranch.SelectedValue.ToString() + " and r.Current_Semester='" + ddlsem.SelectedValue.ToString() + "' and r.cc=0 and r.delflag=0 and r.Exam_Flag<>'debar' and '" + dateto + "' between s.Fromdate and s.Todate " + sec + " " + hourdetails + " order by r.Stud_Name";
        //    dsfeerol = d2.select_method_wo_parameter(strfeeofrollquery, "text");
        ...
        //    }
        //}
        strfeeofrollquery = "select r.Roll_No from stucon s,Registration r where s.roll_no=r.Roll_No and r.Current_Semester=s.semester and s.ack_fee_of_roll=1  and r.Batch_Year='" + batch + "' and r.degree_code=" + degCode + " and r.Current_Semester='" + curSem + "' and r.cc=0 and r.delflag=0 and r.Exam_Flag<>'debar' " + (section != "" ? " and r.sections = '" + section + "'" : string.Empty) + " ";
        dsfeerol = d2.select_method_wo_parameter(strfeeofrollquery, "text");
        for (int fs = 0; fs < dsfeerol.Tables[0].Rows.Count; fs++)
        {
            if (feeofrolls == "")
        ...
        if (feeofrolls.Trim() != "")
        {
            feeofrolls = " and Registration.roll_no not in(" + feeofrolls + ")";
        }

        // hoursvisiblity();
        string sqlstr;

        sqlstr = "select Registration.roll_no,Registration.reg_no, Registration.stud_name,Registration.Roll_admit,Registration.stud_type,registration.serialno,Registration.Adm_Date,delflag,exam_flag,a.app_formno,a.app_no from registration, applyn a where a.app_no=registration.app_no and Registration.Roll_No not in(select s.roll_no from stucon s where s.roll_no=Registration.roll_no and s.semester=Registration.Current_Semester and s.ack_fee_of_roll=1) and registration.degree_code=" + degCode + " and registration.current_semester=" + curSem + " and registration.batch_year=" + batch + "  and RollNo_Flag<>0 and cc=0 " + includedebar + " " + includediscon + " " + (section != "" ? " and Registration.sections = '" + section + "'" : string.Empty) + " " + feeofrolls + " and adm_date<='" + fromDate + "' " + strorder + "";

        DataSet gvds = new DataSet();
        gvds.Clear();
        gvds = d2.select_method_wo_parameter(sqlstr, "text");
        if (gvds.Tables.Count > 0 && gvds.Tables[0].Rows.Count > 0)
        {
            List<System.Decimal> list = gvds.Tables[0].AsEnumerable()
                           .Select(r => r.Field<System.Decimal>("app_no"))
                           .ToList();

            string appNos = string.Join(",", list.ToArray());
            string sql = "select LeaveType as LeaveCode,(select DispText from AttMastersetting  am where LeaveCode=LeaveType and am.CollegeCode='" + collegeCode + "' ) as LeaveDisp,IsHalfDay,HalfTime,HalfDayDate,App_No from SR_Student_Leave_Request where  RequestStatus='1' and App_No in (" + appNos + ") and ( '" + fromDate + "' between  FromDate and ToDate) and ('" + fromDate + "' between FromDate and ToDate) and Collegecode='" + collegeCode + "'";
            DataSet dsLeaveApproved = d2.select_method_wo_parameter(sql, "Text");
            bool lvCheck = false;
            if (dsLeaveApproved.Tables.Count > 0 && dsLeaveApproved.Tables[0].Rows.Count > 0)
            {
                lvCheck = true;
            }

            DataTable dtNewTable = new DataTable();
            dtNewTable.Columns.Add("Admission No");
            dtNewTable.Columns.Add("Roll No");
            dtNewTable.Columns.Add("Reg No");
            //dtNewTable.Columns.Add("App No");
            dtNewTable.Columns.Add("Student Name");
            dtNewTable.Columns.Add("Type");


            DataSet dsLeaveCode = new DataSet();
            dsLeaveCode = d2.select_method_wo_parameter("select DispText from AttMasterSetting where CollegeCode=" + collegeCode + " and CalcFlag='1'", "Text");
            if (dsLeaveCode.Tables.Count > 0 && dsLeaveCode.Tables[0].Rows.Count > 0)
            {
                for (int lcI = 0; lcI < dsLeaveCode.Tables[0].Rows.Count; lcI++)
                {
                    string colName = Convert.ToString(dsLeaveCode.Tables[0].Rows[lcI][0]).Trim();
                    if (!dtNewTable.Columns.Contains(colName))
                        dtNewTable.Columns.Add(colName);
                }
            }

            for (int studI = 0; studI < gvds.Tables[0].Rows.Count; studI++)
            {
                string appNo = Convert.ToString(gvds.Tables[0].Rows[studI]["app_no"]).Trim();
                DataRow dr = dtNewTable.NewRow();
                dr["Roll No"] = Convert.ToString(gvds.Tables[0].Rows[studI]["roll_no"]).Trim();
                dr["Reg No"] = Convert.ToString(gvds.Tables[0].Rows[studI]["reg_no"]).Trim();
                dr["Admission No"] = Convert.ToString(gvds.Tables[0].Rows[studI]["Roll_admit"]).Trim();
                //dr["App No"] = appNo.Trim();
                dr["Student Name"] = Convert.ToString(gvds.Tables[0].Rows[studI]["stud_name"]).Trim();
                dr["Type"] = Convert.ToString(gvds.Tables[0].Rows[studI]["stud_type"]).Trim();

                for (int colI = 5; colI < dtNewTable.Columns.Count; colI++)
                {
                    dr[colI] = "0";
                    if (lvCheck)
                    {
                        string colName = Convert.ToString(dtNewTable.Columns[colI].ColumnName).Trim();
                        dsLeaveApproved.Tables[0].DefaultView.RowFilter = "App_No=" + appNo + " and LeaveDisp='" + colName + "'";
                        DataView dv = dsLeaveApproved.Tables[0].DefaultView;
                        if (dv.Count > 0)
                        {
                            dr[colI] = dv[0]["LeaveDisp"];
                        }
                    }
                }
            ...
            gridPopAttnd.Visible = true;
            gridPopAttnd.DataSource = dtNewTable;
            gridPopAttnd.DataBind();
            Session["dt"] = dtNewTable;
            //gridPopAttnd.Rows[gridPopAttnd.Rows.Count - 2].Cells[0].Text = " ";
            //gridPopAttnd.Rows[gridPopAttnd.Rows.Count - 1].Cells[0].Text = " ";
        }
        else
        {
            if (Session["dt"] != null) Session.Remove("dt");
            gridPopAttnd.Visible = false;
        }
    }

    protected void gridPopAttnd_OnRowDataBound(object sender, GridViewRowEventArgs e)
    {
        int tot = 6;
        //if (Session["Rollflag"] != null && Session["Rollflag"].ToString().Trim() == "0")
        //{
        //    e.Row.Cells[2].Visible = false;
        //    tot--;
        //}
        //if (Session["Regflag"] != null && Session["Regflag"].ToString().Trim() == "0")
        //{
        //    e.Row.Cells[2].Visible = false;
        //    tot--;
        //}
        //if (Session["AdmissionNo"] != null && Session["AdmissionNo"].ToString().Trim() == "0")
        //{
        ...
        if (e.Row.RowType == DataControlRowType.Header)
        {
            ...
            for (int i = 6; i < e.Row.Cells.Count; i++)
            {
                ...
                txt.Visible = true;
                txt.Attributes.Add("onclick", "checkvalueHeader(" + (i - 6) + "," + tot + ")");
        ...
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            for (int i = 6; i < e.Row.Cells.Count; i++)
            {
                ...
                chk.Height = 30;
                chk.Width = 30;
                chk.Attributes.Add("onclick", "Check_Click(this,'" + (e.Row.RowIndex + 1) + "'," + i + "," + tot + ");");

                if (e.Row.Cells[i].Text.Trim() != "0")
                {
                    e.Row.Enabled = false;
                    e.Row.BackColor = ColorTranslator.FromHtml("#00A6C0"); ;
                    mouseOver = false;
            ...
            //e.Row.Cells[1].Attributes["onclick"] = "checkvalue('" + (e.Row.RowIndex) + "')";
            //e.Row.Cells[2].Attributes["onclick"] = "checkvalue('" + (e.Row.RowIndex) + "')";
            //e.Row.Cells[3].Attributes["onclick"] = "checkvalue('" + (e.Row.RowIndex) + "')";
            //e.Row.Cells[4].Attributes["onclick"] = "checkvalue('" + (e.Row.RowIndex) + "')";
            if (mouseOver)
            {
    ...
    protected void gridPopAttnd_DataBound(object sender, EventArgs e)
    {
        int tot = 6;
        //if (Session["Rollflag"] != null && Session["Rollflag"].ToString().Trim() == "0")
        //{
        ...
        //    tot--;
        //}
        GridViewRow gr = gridPopAttnd.HeaderRow;
        for (int i = 6; i < gr.Cells.Count; i++)
        {
            gr.Cells[i].Attributes.Add("onclick", "checkvalueHeader(" + (i - 6) + "," + tot + ")");

        }
        //gridPopAttnd.Columns[4].Visible = false;
    }
    //Save
    protected void Button2_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            foreach (GridViewRow gRow in gridPopAttnd.Rows)
            {
                string LeaveVal = string.Empty;

                for (int col = 6; col < gRow.Cells.Count; col++)
                {
                    CheckBox chk = (CheckBox)gRow.Cells[col].FindControl("chk_" + col);
                    if (chk != null && chk.Checked)
                    {
                        LeaveVal = gridPopAttnd.HeaderRow.Cells[col].Text.Trim();
                        break;
                    }
                }
                if (LeaveVal != string.Empty)
                {
                    string rollnos = gRow.Cells[2].Text.Trim() + "*" + LeaveVal;
                    if (ddlentry.SelectedItem.Text.ToLower() == "admission no")
                    {
                        rollnos = gRow.Cells[1].Text.Trim() + "*" + LeaveVal;
                    }
                    if (ddlentry.SelectedItem.Text.Trim().ToLower() == "reg no")
                    {
                        //string regNo = da.GetFunctionv("select roll_no from Registration where roll_no='" + gRow.Cells[2].Text.Trim() + "'");
                        //if (!string.IsNullOrEmpty(regNo.Trim()))
                        //{
                        //    rollnos = regNo.Trim() + "*" + LeaveVal;
                        //}
                        rollnos = gRow.Cells[3].Text.Trim() + "*" + LeaveVal;
                    }
                    if (sqlsec == "")
                    {
                        sqlsec = "" + rollnos + "";
    ...
    protected void btnxl_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            string strquery1 = "Select * from AttMasterSetting where calcflag='1' and CollegeCode='" + ddlcollege.SelectedValue.ToString() + "' order by DispText desc";
            attnmas = d2.select_method(strquery1, hat, "Text");

            Hashtable htLeaveCodes = new Hashtable();
            if (attnmas.Tables.Count > 0 && attnmas.Tables[0].Rows.Count > 0)
            {
                ...
                for (int lC = 0; lC < attnmas.Tables[0].Rows.Count; lC++)
                {
                    string CName = Convert.ToString(attnmas.Tables[0].Rows[lC]["DispText"]).ToUpper();
                    if (!htLeaveCodes.Contains(CName))
                    {
                        htLeaveCodes.Add(CName, Convert.ToString(attnmas.Tables[0].Rows[lC]["leaveCode"]));
                    }
                }
            }
            else
            ...
            string sqlquery1 = "select distinct Batch_Year,degree_code,Current_Semester,ltrim(rtrim(isnull(Sections,''))) as Sections,Roll_No,Reg_No,dbo.ProperCase(Stud_Name) as Stud_Name,Roll_Admit,app_no  from Registration  where  RollNo_Flag<>0 and cc=0 and exam_flag <> 'DEBAR' and delflag=0;select *  from attendance where month_year='" + month_year + "'";
            ...
            foreach (GridViewRow gvrow in gvuserodrlist.Rows)
            {
                ...
                if (txtabbsentnewA != "")
                {
                    ...
                    #region Idhris 24-11-2016
                    Dictionary<string, string> dicAbsentees = new Dictionary<string, string>();

                    foreach (string absRol in abbsentroll)
                    {
                        try
                        {
                            string[] splitval = absRol.Split('*');
                            string lCode = htLeaveCodes.Contains(splitval[1].ToUpper()) ? htLeaveCodes[splitval[1].ToUpper()].ToString() : "1";
                            if (!dicAbsentees.ContainsKey(splitval[0]))
                                dicAbsentees.Add(splitval[0].ToLower(), lCode);
                        }
                        catch { 
                        }
                    }

                    foreach (KeyValuePair<string, string> abseRol in dicAbsentees)
                    {
                        if (!dicabsen.ContainsKey(abseRol.Key.ToString().Trim().ToLower()))
                        {
                            dicabsen.Add(abseRol.Key.ToString().Trim().ToLower(), abseRol.Value.ToString().Trim().ToLower());
                        }
                    }
                    ...
                    for (int itemcount = 0; itemcount < chklssec.Items.Count; itemcount++)
                    {
                        if (chklssec.Items[itemcount].Selected == true)
                        {
                            ...
                            if (loadhour12.Contains(sqlsec))
                            {
                                if (Att_dcolumnpinsertr == "" || Att_dcolumnpinsert == "" || Att_dcolumnp == "")
                                {
                    ...
                    if (dsselect14.Tables.Count > 0 && dsselect14.Tables[0].Rows.Count > 0)
                    {
                    ...
                    if (dv2.Count > 0)
                    {
                        List<System.Decimal> list = dv2.ToTable().AsEnumerable()
                                                       .Select(r => r.Field<System.Decimal>("app_no"))
                                                       .ToList();

                        string appNos = string.Join(",", list.ToArray());
                        string collegeCode = ddlcollege.Items.Count > 0 ? ddlcollege.SelectedValue : "13";
                        string sql = "select LeaveType as LeaveCode,(select DispText from AttMastersetting  am where LeaveCode=LeaveType and am.CollegeCode='" + collegeCode + "' ) as LeaveDisp,IsHalfDay,HalfTime,HalfDayDate,App_No from SR_Student_Leave_Request where  RequestStatus='1' and App_No in (" + appNos + ") and ( '" + chechfromdate + "' between  FromDate and ToDate) and ('" + chechfromdate + "' between FromDate and ToDate) and Collegecode='" + collegeCode + "'";
                        DataSet dsLeaveApproved = d2.select_method_wo_parameter(sql, "Text");
                        bool lvCheck = false;
                        if (dsLeaveApproved.Tables.Count > 0 && dsLeaveApproved.Tables[0].Rows.Count > 0)
                        {
                        ...
                        {
                            #region check fro leave approved

                            string app_No = dv2[dc]["app_No"].ToString().ToLower();
                            DataView dvLvApp = new DataView();
                            if (lvCheck)
                            {
                                dsLeaveApproved.Tables[0].DefaultView.RowFilter = "App_No=" + app_No + "";
                                dvLvApp = dsLeaveApproved.Tables[0].DefaultView;
                            }
                            ...
                            if (dicAbsentees.ContainsKey(chkroll.ToLower()) || dvLvApp.Count > 0)//abbsentroll.Contains(chkroll.ToLower())
                            {
                                if (dicabsen.ContainsKey(chkroll.Trim().ToLower()))
                                {
                                    dicabsen.Remove(chkroll.Trim().ToLower());
                                }
                                hassabbrollno = app_No; //rollno.ToString();

                                if (dvLvApp.Count > 0)
                                {
                                    LeaveCodea = Convert.ToString(dvLvApp[0]["LeaveCode"]);
                                }
                                else
                                {
                                    LeaveCodea = dicAbsentees[chkroll.ToLower()];
                                }
                                Att_dcolumna = string.Empty;
                                Att_dcolumnainsertr = string.Empty;
                                Att_dcolumnainsert = string.Empty;
                                ...
                                for (int itemcount = 0; itemcount < chklssec.Items.Count; itemcount++)
                                {
                                    if (chklssec.Items[itemcount].Selected == true)
                                    {
                                        ...
                                        if (loadhour12.Contains(sqlsec))
                                        {
                                            if (Att_dcolumna == "" || Att_dcolumnainsertr == "" || Att_dcolumnainsert == "")
                                            {
                                ...
                                //string sqlquery = "if exists(select * from attendance where Att_App_no in('" + hassabbrollno + "') and month_year ='" + month_year + "') update attendance set " + Att_dcolumna + " where Att_App_no='" + hassabbrollno + "' and month_year ='" + month_year + "' else insert into attendance(Att_App_no,Att_CollegeCode,roll_no,month_year," + Att_dcolumnainsert + ") values('" + hassabbrollno + "','" + collegeCode + "','" + rollno + "','" + month_year + "'," + Att_dcolumnainsertr + ")";
                                //int save = d2.insert_method(sqlquery, hat, "Text");


                                hat.Clear();
                                hat.Add("Att_App_no", hassabbrollno);
                                hat.Add("Att_CollegeCode", collegeCode.ToString());
                                hat.Add("rollno", rollno);
                                hat.Add("monthyear", month_year);
                                hat.Add("columnname", Att_dcolumnainsert);
                                hat.Add("colvalues", Att_dcolumnainsertr);
                                hat.Add("coulmnvalue", Att_dcolumna);
                                int save = d2.insert_method("sp_ins_upd_student_attendance_Dead", hat, "sp");

                                if (save > 0)
                                {
                                    //if (hrcheck)
                                    attendanceMark(app_No, month_year, strPerDays, noMaxHrsDay, noFstHrsDay, noSndHrsDay, noMinFstHrsDay, noMinSndHrsDay, DateVal, ddlcollege.SelectedValue.ToString(), LeaveCodea);
                                ...
                                if (chksms.Checked == true)
                                {
                                    ...
                                    if (hoers.Tables[0].Rows.Count > 0)
                                    {
                                        ...
                                        if (permsson.Trim() != "" || allhour == "1")
                                        {
                                            ...
                                            if (single_hour != "")
                                            {
                                                SendingSms(rollno, hassabbrollno, regno, admissionno, chechfromdate1, single_hour, collacronym, newdeptcource, sections, LeaveCodea);
                                            }
                                        }
                                    }
                                }
                            ...
                            else if (loadhour123.Contains(rollno.ToLower()))
                            {
                                ...
                                //string sqlquery = "if exists(select * from attendance where Att_App_no in ('" + app_No + "') and Roll_No in(" + hassprerollno + ") and month_year ='" + month_year + "') update attendance set " + Att_dcolumnp + " where Att_App_no=" + app_No + " and month_year ='" + month_year + "' else insert into attendance(Att_App_no,Att_CollegeCode,Roll_No,month_year," + Att_dcolumnpinsert + ") values(" + app_No + "," + collegeCode + "," + hassprerollno + ",'" + month_year + "'," + Att_dcolumnpinsertr + ")";
                                //int save = d2.insert_method(sqlquery, hat, "Text");
                                ...
                                hat.Add("rollno", rollno);
                                hat.Add("monthyear", month_year);
                                hat.Add("columnname", Att_dcolumnpinsert);
                                hat.Add("colvalues", Att_dcolumnpinsertr);
                                hat.Add("coulmnvalue", Att_dcolumnp);
                                int save = d2.insert_method("sp_ins_upd_student_attendance_Dead", hat, "sp");

                                if (save > 0)
                                {
                                    //if (hrcheck)
                                    attendanceMark(app_No, month_year, strPerDays, noMaxHrsDay, noFstHrsDay, noSndHrsDay, noMinFstHrsDay, noMinSndHrsDay, DateVal, ddlcollege.SelectedValue.ToString(), LeaveCodep);
                                    savsotnot = true;
                                }
                            }
                        }
