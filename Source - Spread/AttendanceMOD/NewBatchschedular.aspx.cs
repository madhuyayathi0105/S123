...
public partial class NewBatchschedular : System.Web.UI.Page
{
    ...
    protected void ddlsection_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        DropDownList ddlcollparent = (DropDownList)this.user_control.FindControl("ddlcollege");
        ...
        if (ds_cssem.Tables[0].Rows.Count > 0)
        {
            ...
            if (Convert.ToString(currentsem) == Convert.ToString(ddlsemparent.SelectedValue.ToString()))
            {
                if (ddltimetable != null)
                {
                    ...
                    //string batchquery = " select TTName as TT_name, convert(varchar(15),FromDate,103) as TT_date from Semester_Schedule where degree_code='" + ddlbranchparent.SelectedValue.ToString() + "' and batch_year='" + ddlbatchparent.SelectedValue.ToString() + "' " + strsec + " and semester='" + ddlsemparent.SelectedValue.ToString() + "'";
                    //ds_batchs = obi_access.select_method_wo_parameter(batchquery, "text");

                    string strtimetable = "select distinct TT_name, convert(varchar(15),TT_date,103) as TT_date from TT_ClassTimetable ct, TT_ClassTimetableDet ctd where ct.TT_ClassPK = ctd.TT_ClassFk and ct.TT_colCode='" + Convert.ToString(ddlcollparent.SelectedValue) + "' and ct.TT_degCode='" + ddlbranchparent.SelectedValue + "' and ct.TT_batchyear='" + ddlbatchparent.SelectedValue + "' and ct.TT_sem='" + ddlsemparent.SelectedValue + "'  and TT_Sec='" + ddlsection.SelectedValue.ToString() + "' ";
                    ds_batchs = d2.select_method_wo_parameter(strtimetable, "Text");
                    ...
                    if (ds_batchs.Tables[0].Rows.Count > 0)
                    {
                        ...
                        {
                            object value = row["TT_name"];
                            object value1 = row["TT_date"];
    ...
    protected void btnGo_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            else
            {
                ...
                if (serialflag == true)
                {
                    stu_namequery = "select roll_no as rollno, stud_name as studentname  from registration where degree_code='" + ddlbranchvalue.ToString() + "' and batch_year='" + ddlbatchvalue.ToString() + "' " + strsec + " and current_semester='" + ddlsemvalue.ToString() + "' and RollNo_Flag<>0 and cc=0 and delflag=0 and exam_flag <> 'DEBAR' order by serialno";
                }
                else
                {
                ...
                if (ds_stu_names.Tables.Count > 0 && ds_stu_names.Tables[0].Rows.Count > 0)
                {
                    ...
                    for (int i = 0; i < ds_stu_names.Tables[0].Rows.Count; i++)
                    {
                        ...
                        batch_spread.Sheets[0].RowCount++;
                        string selectedbatch = "select distinct batch from subjectchooser where roll_no='" + rollno + "' and semester='" + ddlsemvalue.ToString() + "' and batch is not null and batch<>''";
                    ...
                    ArrayList daylistNew = new ArrayList();
                    daylistNew.Add("Monday");
                    daylistNew.Add("Tuesday");
                    daylistNew.Add("Wednesday");
                    daylistNew.Add("Thursday");
                    daylistNew.Add("Friday");
                    daylistNew.Add("Saturday");
                    daylistNew.Add("Sunday");
                    ...
                    if (ds_syllcode.Tables.Count > 0 && ds_syllcode.Tables[0].Rows.Count > 0)
                    {
                        ...
                        string subjectnumber = "Select subjecT_no,subjecT_code from subject,sub_sem where sub_sem.subtype_no = subject.subtype_no and (sub_sem.Lab = 1 or sub_sem.projThe=1) and sub_sem.syll_code = subject.syll_code and subject.syll_code='" + getsyllcode + "'";
                        ...
                        //Added by Idhris 13-06-2017
                        DataTable dtTTLab = dirAcc.selectDataTable(" select distinct cast(TT_subno  as varchar(20)) as TT_subno,TT_Day,TT_Hour,Daydiscription,TT_staffcode from TT_ClassTimetable ct, TT_ClassTimetableDet ctd, subject s,sub_sem ss,TT_Day_Dayorder do where do.TT_Day_DayorderPK=TT_Day and ct.TT_ClassPK = ctd.TT_ClassFk and TT_subno = s.subject_no and s.syll_code=ss.syll_code and s.subType_no = ss.subType_no  and ISNULL(ss.Lab,'0')='1'     and TT_colCode='" + ddlcollegevalue + "' and TT_batchyear='" + ddlbatchvalue + "' and TT_degCode='" + ddlbranchvalue + "' and TT_sem='" + ddlsemvalue.ToString() + "' and TT_sec='" + sections + "' and TT_name='" + ddltimetable.SelectedItem.ToString().Split(new Char[] { '@' })[0].Trim() + "' and TT_date='" + (date_fm1[1].ToString() + "/" + date_fm1[0].ToString() + "/" + date_fm1[2].ToString()) + "' ");
                        DataTable dtLabOnly = dtTTLab.DefaultView.ToTable(true, "TT_subno");
                        if (dtLabOnly.Rows.Count > 0)
                        {
                            List<string> lstTT_lab = dtLabOnly.AsEnumerable().Select(r => r.Field<string>("TT_subno")).ToList<string>();
                            validsunno = string.Join(",", lstTT_lab);
                        }

                        #region New Timetable to old format
                        DataTable dtNewTT = new DataTable();
                        dtNewTT.Columns.Add("degree_code");
                        dtNewTT.Columns.Add("class_advisor");
                        dtNewTT.Columns.Add("Semester");
                        dtNewTT.Columns.Add("batch_year");
                        dtNewTT.Columns.Add("TTName");
                        dtNewTT.Columns.Add("FromDate");
                        dtNewTT.Columns.Add("LastRec");
                        dtNewTT.Columns.Add("Sections");

                        DataRow drN = dtNewTT.NewRow();
                        drN["degree_code"] = ddlbranchvalue;
                        drN["class_advisor"] = "";
                        drN["Semester"] = ddlsemvalue.ToString();
                        drN["batch_year"] = ddlbatchvalue;
                        drN["TTName"] = ddltimetable.SelectedItem.ToString().Split(new Char[] { '@' })[0].Trim();
                        drN["FromDate"] = (date_fm1[1].ToString() + "/" + date_fm1[0].ToString() + "/" + date_fm1[2].ToString());
                        drN["LastRec"] = "1";
                        drN["Sections"] = sections;
                        hatsubject.Clear();
                        for (int daYI = 0; daYI < daylist.Count; daYI++)
                        {
                            for (int hRsI = 1; hRsI <= numbhrs; hRsI++)
                            {
                                string colName = daylist[daYI] + hRsI.ToString();
                                dtNewTT.Columns.Add(colName);

                                dtTTLab.DefaultView.RowFilter = "Daydiscription='"+daylistNew[daYI]+"' and TT_Hour='"+hRsI+"'";
                                DataTable dtCurLab = dtTTLab.DefaultView.ToTable();
                                if (dtCurLab.Rows.Count > 0)
                                {
                                    StringBuilder sb = new StringBuilder();
                                    for (int lbCnt = 0; lbCnt < dtCurLab.Rows.Count; lbCnt++)
                                    {
                                        string subno = Convert.ToString(dtCurLab.Rows[lbCnt]["TT_subno"]);
                                        string stafCode = Convert.ToString(dtCurLab.Rows[lbCnt]["TT_staffcode"]);
                                        sb.Append(subno+"-"+stafCode+"-"+"L;");

                                        if (!hatsubject.Contains(subno))
                                        {
                                            hatsubject.Add(subno, colName);
                                        }
                                        else
                                        {
                                            string prevVal = hatsubject[subno].ToString();
                                            hatsubject[subno] = prevVal + "," + colName;
                                        }
                                    }
                                    drN[colName] = sb.ToString().TrimEnd(';');
                                }
                            }
                        }
                        dtNewTT.Rows.Add(drN);
                        for (int tblI = 0; tblI < ds_periodallot.Tables.Count; tblI++)
			            {
                            ds_periodallot.Tables.RemoveAt(0);
			            }
                        ds_periodallot.Tables.Add(dtNewTT);
                        #endregion

                        //Ended by Idhris
                        #region Commented by Idhris
