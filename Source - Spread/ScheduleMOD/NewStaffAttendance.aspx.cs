...
public partial class AttendanceMOD_NewStaffAttendance : System.Web.UI.Page
{
            ...
            while (dtTo >= dtFrom)
            {
                ...
                foreach (DataRow drClassTT in dtDistinctClassTT.Rows)
                {
                    ...
                    if (dtTTPks.Rows.Count > 0)
                    {
                        ...
                        {
                            //if (ttCount == 0)                           
                            //    dtStaffTT = dtOvalStaffTT.DefaultView.ToTable();
                            //else                            
                ...
                for (byte curPeriod = 1; curPeriod <= maxPeriod; curPeriod++)
                {
                    ...
                    if (dtHourTTDet.Rows.Count > 0)
                    {
                        ...
                        foreach (DataRow drAlter in dtHourTTDet.Rows)
                        {
                            ...
                            if (isStfAlter)
                            {
                                dtStaffAlterTT.DefaultView.RowFilter = "TT_Day='" + dicDayOrder[dayOfWeek] + "' and TT_Hour='" + curPeriod + "' and TT_AlterDate='" + dtFrom.ToString("MM/dd/yyyy") + "' and ((TT_batchyear='" + batchHr + "' and TT_colCode='" + collegeHr + "' and TT_degCode='" + degreeHr + "' and TT_sem='" + semHr + "' and TT_sec='" + secHr + "') or (TT_batchyear not in('" + string.Join("','", batchYearList.ToArray()) + "') and TT_degCode not in('" + string.Join("','", degreeCodeList.ToArray()) + "') and TT_sem not in('" + string.Join("','", semesterList.ToArray()) + "') and TT_sec not in('" + string.Join("','", sectionsList.ToArray()) + "')))";
                            ...
                            if (!alterSchd)
                            {
                                ...
                                if (isOthStfAlter)
                                {
                                    dtOtrStfAlterTT.DefaultView.RowFilter = "TT_Day='" + dicDayOrder[dayOfWeek] + "' and TT_Hour='" + curPeriod + "' and TT_AlterDate='" + dtFrom.ToString("MM/dd/yyyy") + "' and ((TT_batchyear='" + batchHr + "' and TT_colCode='" + collegeHr + "' and TT_degCode='" + degreeHr + "' and TT_sem='" + semHr + "' and TT_sec='" + secHr + "') or (TT_batchyear not in('" + string.Join("','", batchYearList.ToArray()) + "') and TT_degCode not in('" + string.Join("','", degreeCodeList.ToArray()) + "') and TT_sem not in('" + string.Join("','", semesterList.ToArray()) + "') and TT_sec not in('" + string.Join("','", sectionsList.ToArray()) + "')))";
                ...
                if (lnkPeriod != null && lnkPeriod.Text.Trim() != string.Empty && !lnkPeriod.Text.Contains("##MISD"))
                {
                    ...
                    if (subDets.Length > 0 && !string.IsNullOrEmpty(subDets[0].Trim()))
                    {
                        for (int subI = 0; subI < subDets.Length; subI++)
                        {
                            ...
                            dtStudData.Merge(getStudentData(collegeCode, batch, degcode, sem, sec, subno, staffcode, elective, lab, labDay, coli.ToString(), ttClasspk[0], ttClasspk[1], lblCurDate.Text));
                            if (isAlter)
                                break;
    ...
    protected void lnkAttMark(object sender, EventArgs e)
    {
        try
        {
            ...
            string[] classttPks = lblTTclasspk.Text.Split(';');
            for (int subI = 0; subI < subDets.Length; subI++)
            {
                string[] ttData = subDets[subI].Split('#');
                string[] ttDispData = subDispDets[subI].Split('$');
                ...
                dtStudData.Merge(getStudentData(collegeCode, batch, degcode, sem, sec, subno, staffcode, elective, lab, labDay, colIndx.ToString(), ttClasspk[0], ttClasspk[1], lblCurDate.Text));
            }
    ...
    protected void ddlMultiSub_OnChanged(object sender, EventArgs e)
    {
        try
        {
            ...
            for (int subI = 0; subI < subDets.Length; subI++)
            {
                ...
                dtStudData.Merge(getStudentData(collegeCode, batch, degcode, sem, sec, subno, staffcode, elective, lab, labDay, hour.ToString(), ttClasspk[0], ttClasspk[1], curDate));
            }
    ...
    private DataTable getStudentData(string colCode, string batch, string degcode, string cursem, string sec, string subno, string staffcode, string elect, string lab, string labDay, string labhr, string ttname, string isAlter, string curDate)
    {
        ...
        try
        {
            ...
            string selQ = " select sc.staffcode,a.app_no,r.stud_name,a.app_formno,r.Roll_No,r.Roll_Admit,r.Reg_No,case when isnull(r.Stud_Type,'Day Scholar')='Day Scholar' then 'D' else case when ISNULL(rd.Room_Name,'')='' then 'H' else 'H - '+ISNULL(rd.Room_Name,'') end  end  as Resident from applyn a, subjectChooser sc ,staff_selector ss,Registration r left join HT_HostelRegistration hr on hr.APP_No=r.App_No left  join Room_Detail rd on RoomPK=hr.RoomFK where sc.subject_no=ss.subject_no and r.App_No = a.app_no and r.Current_Semester=sc.semester and r.Roll_No = sc.roll_no and sc.subject_no='" + subno + "' and ss.staff_code  like '%" + staffcode + "%'  and r.college_code='" + colCode + "' and r.Batch_Year in ('" + batch + "') and r.degree_code in ('" + degcode + "') and r.Current_Semester in ('" + cursem + "')  " + qrySection + qryStaffSection + qryStudeStaffSelector + orderBy;
            ...
            if (elect == "true")
            {
                ...
                selQ = " select sc.staffcode,a.app_no,r.stud_name,a.app_formno,r.Roll_No,r.Roll_Admit,r.Reg_No,case when isnull(r.Stud_Type,'Day Scholar')='Day Scholar' then 'D' else case when ISNULL(rd.Room_Name,'')='' then 'H' else 'H - '+ISNULL(rd.Room_Name,'') end  end  as Resident from  applyn a, subjectChooser sc ,staff_selector ss,Registration r left join HT_HostelRegistration hr on hr.APP_No=r.App_No left  join Room_Detail rd on RoomPK=hr.RoomFK where sc.subject_no=ss.subject_no and r.App_No = a.app_no and r.Current_Semester=sc.semester and r.Roll_No = sc.roll_no and r.Batch_Year=ss.batch_year and sc.subject_no='" + subno + "' and ss.staff_code  like '%" + staffcode + "%' and r.college_code='" + colCode + "' and r.Batch_Year in ('" + batch + "') and r.degree_code in ('" + degcode + "') and r.Current_Semester in ('" + cursem + "')   " + qrySection + qryStaffSection + qryStudeStaffSelector + orderBy;
