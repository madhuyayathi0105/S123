...
public partial class CoeMod_ExamAttendanceReportNew : System.Web.UI.Page
{
    ...
    public void Bindexamdate()
    {
        try
        {
            ...
            string getexamdate = "select distinct convert(varchar(10),exdt.Exam_date,105) as exam_date,exdt.Exam_date from exmtt_det as exdt,exmtt as exm,degree d,Course c where exm.exam_code=exdt.exam_code and exm.degree_code=d.Degree_Code and d.Course_Id=c.Course_Id " + strtype + " and exm.exam_month=" + ddlMonth.SelectedValue.ToString() + " and exm.exam_year=" + ddlYear.SelectedValue.ToString() + " and DATEPART(month, exdt.exam_date)>='" + ddlMonth.SelectedValue.ToString() + "' and DATEPART(YEAR, exdt.exam_date)>='" + ddlYear.SelectedItem.Text.ToString() + "' order by exdt.Exam_date";
        ...
        try
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    string spreadbind1 = "select distinct es.roomno,COUNT(1) as strength,es.ses_sion,es.edate,ed.degree_code,ed.batch_year,es.subject_no,es.bundle_no,s.subject_code,cm.priority,cm.Mode  from registration r,exam_details ed,exam_application ea,exam_appl_details ead,exam_seating as es,subject s,class_master cm where  cm.rno=es.roomno and ed.exam_code=ea.exam_code and ea.appl_no=ead.appl_no and ea.roll_no=r.roll_no and r.exam_flag<>'Debar' and es.regno=r.Reg_No and ead.subject_no=es.subject_no and s.subject_no=ead.subject_no and s.subject_no=ead.subject_no and   ed.Exam_Month='" + ddlMonth.SelectedValue.ToString() + "' and ed.Exam_year='" + ddlYear.SelectedValue.ToString() + "' and es.roomno in ('" + departnt + "') and es.edate='" + examdate + "' " + str_degrcode + " " + sessiond + "  group by cm.Mode,cm.priority,es.roomno,es.ses_sion,es.edate ,ed.degree_code,ed.batch_year,es.subject_no,es.bundle_no,s.subject_code order by cm.Mode,cm.priority ";


                    DataSet ds2 = d2.select_method_wo_parameter(spreadbind1, "Text");

                    qry = "select es.edate,es.ses_sion,es.roomno,es.subject_no,es.regno,es.degree_code,ed.Exam_Month,ed.Exam_year,r.Batch_Year,r.roll_no,es.seat_no,es.bundle_no,s.subject_code from Exam_Details ed,exam_application ea,exam_appl_details ead ,exam_seating es,Registration r,subject s  where ed.exam_code=ea.exam_code and ea.appl_no=ead.appl_no and ead.subject_no=es.subject_no and r.Roll_No=ea.roll_no and ed.degree_code=r.degree_code and ed.batch_year=r.Batch_Year and ed.degree_code=es.degree_code and es.degree_code=r.degree_code and s.subject_no=es.subject_no and es.regno=r.Reg_No and es.edate='" + examdate + "' " + strdegrcode + " " + sessiond + " and es.roomno in ('" + departnt + "')  and ed.Exam_Month='" + ddlMonth.SelectedValue.ToString() + "' and ed.Exam_year='" + ddlYear.SelectedValue.ToString() + "' ";
                    ...
                    string notebatch = string.Empty;
                    string notedegree = string.Empty;
                    string notesubjectNo = string.Empty;
                    DataTable dicnotes=new DataTable();
                    dicnotes = ds2.Tables[0].DefaultView.ToTable(true, "degree_code","batch_year", "roomno", "subject_code");//"bundle_no"
                    ...
                    if (ds2.Tables.Count > 0 && ds2.Tables[0].Rows.Count > 0)
                    {
                        ...
                        for (int i = 0; i < ds2.Tables[0].Rows.Count; i++)
                        {
                            string conbundleNo = string.Empty;
                            ...
                            sesson = ds2.Tables[0].Rows[i]["ses_sion"].ToString();
                            exdate = ds2.Tables[0].Rows[i]["edate"].ToString();
                            sbjno = ds2.Tables[0].Rows[i]["subject_code"].ToString();
                            dicnotes.DefaultView.RowFilter = "roomno='" + roomno + "' and subject_code='" + sbjno + "'";
                            DataTable dtbatch = dicnotes.DefaultView.ToTable();
                            if (dtbatch.Rows.Count > 0)
                            {
                                notebatch = string.Empty;
                                notedegree = string.Empty;
                                notesubjectNo = string.Empty;
                                foreach (DataRow drDegreeName in dtbatch.Rows)
                                {
                                    if(string.IsNullOrEmpty(notebatch))
                                        notebatch = Convert.ToString(drDegreeName["batch_year"]).Trim();
                                    else if (!notebatch.Contains(Convert.ToString(drDegreeName["batch_year"]).Trim()) && !string.IsNullOrEmpty(notebatch))
                                        notebatch = notebatch+"','"+Convert.ToString(drDegreeName["batch_year"]).Trim();

                                    if (string.IsNullOrEmpty(notedegree))
                                        notedegree = Convert.ToString(drDegreeName["degree_code"]).Trim();
                                    else if (!notedegree.Contains(Convert.ToString(drDegreeName["degree_code"]).Trim()) && !string.IsNullOrEmpty(notedegree))
                                        notedegree = notedegree + "','" + Convert.ToString(drDegreeName["degree_code"]).Trim();

                                    //if (string.IsNullOrEmpty(notesubjectNo))
                                    //    notesubjectNo = Convert.ToString(drDegreeName["degree_code"]).Trim();
                                    //else if (!notesubjectNo.Contains(Convert.ToString(drDegreeName["degree_code"]).Trim()))
                                    //    notesubjectNo = notesubjectNo + "," + Convert.ToString(drDegreeName["degree_code"]).Trim();
                                }
                            }
                            ...
                            if (notedegree.Contains("','"))
                            {
                                string[] degrrcode1 = notedegree.Split(new string[] { "','" }, StringSplitOptions.RemoveEmptyEntries);
                                //string[] degrrcode1 = notedegree.Split(',');
                                degcode =Convert.ToString(degrrcode1[0]);
                            }
                            else
                            {
                                degcode = notedegree;
                            ...
                            if (dicDegreeName.ContainsKey(degcode.ToLower()))
                            {
                                departmentName = dicDegreeName[degcode.ToLower()];
                                dept = departmentName;
                            }
                            if (!ht.ContainsKey(sbjno + '-' + roomno))
                            {
                                ...
                                AttSpread.Sheets[0].Cells[AttSpread.Sheets[0].RowCount - 1, 1].CellType = cheall;
                                AttSpread.Sheets[0].Cells[AttSpread.Sheets[0].RowCount - 1, 8].Text = notedegree;
                                AttSpread.Sheets[0].Cells[AttSpread.Sheets[0].RowCount - 1, 8].Note = notedegree;//temp
                                AttSpread.Sheets[0].Cells[AttSpread.Sheets[0].RowCount - 1, 2].Text = roomno;
                                AttSpread.Sheets[0].Cells[AttSpread.Sheets[0].RowCount - 1, 3].Note = notebatch;
                                AttSpread.Sheets[0].Cells[AttSpread.Sheets[0].RowCount - 1, 3].Text = dept;
                                ds2.Tables[0].DefaultView.RowFilter = "subject_code='" + sbjno + "' and edate='" + exdate + "' and ses_sion='" + sesson + "' and roomno='" + roomno + "'";
                                ...
                                for (int st = 0; st < dvstucount.Count; st++)
                                {
                                    ...
                                    if (nodtubun < stuco)
                                    {
                                        strength = Convert.ToString(nodtubun);
                                    }
                                    else
                                    {
                                ...
                                AttSpread.Sheets[0].Cells[AttSpread.Sheets[0].RowCount - 1, 4].Text = stuco.ToString();
                            ...
                            string cnt = "select es.edate,es.ses_sion,es.roomno,es.subject_no,es.regno,es.degree_code,r.Batch_Year,r.roll_no,s.subject_code from Exam_Details ed,exam_application ea,exam_appl_details ead ,exam_seating es,Registration r,subject s where ed.exam_code=ea.exam_code and ea.appl_no=ead.appl_no and ead.subject_no=es.subject_no and r.Roll_No=ea.roll_no and ed.degree_code=r.degree_code and ed.batch_year=r.Batch_Year and ed.degree_code=es.degree_code and es.degree_code=r.degree_code and es.regno=r.Reg_No and es.edate='" + exdate + "' and es.ses_sion ='" + sesson + "'  and roomno='" + roomno + "' and and es.subject_no =s.subject_no and s.subject_code='"+sbjno+"' and ed.Exam_Month='" + ddlMonth.SelectedValue.ToString() + "' and ed.Exam_year='" + ddlYear.SelectedValue.ToString() + "' order by es.seat_no";//and es.degree_code='" + degrrcode + "' and r.batch_year='" + batchyr + "'
                            ...
                            if (dsStudentDetails.Tables.Count > 0 && dsStudentDetails.Tables[0].Rows.Count > 0)
                            {
                                ...
                                dsStudentDetails.Tables[0].DefaultView.RowFilter = "Exam_year='" + Convert.ToString(ddlYear.SelectedValue).Trim() + "' and Exam_Month='" + Convert.ToString(ddlMonth.SelectedValue).Trim() + "' and ses_sion ='" + sesson + "' and subject_code  ='" + sbjno + "' and edate='" + exdate + "' and roomno='" + roomno + "'";//and batch_year='" + batchyr + "' and degree_code='" + degrrcode + "'
                            ...
                            if (dsv.Tables.Count > 0 && dsv.Tables[0].Rows.Count > 0)
                            {
                                if (!ht.ContainsKey(sbjno + '-' + roomno))
                                {
                                    ...
                                    {
                                        string subjnoNew = Convert.ToString(dsv.Tables[0].Rows[k]["subject_no"]).Trim();
                                        string newBundleNo = Convert.ToString(dsv.Tables[0].Rows[k]["bundle_no"]).Trim();
                                        if (string.IsNullOrEmpty(newBundleNo))
                                        {
                                            bundleno = d2.GetFunction("select bundle_no from exam_seating where regno='" + dsv.Tables[0].Rows[k]["regno"].ToString() + "' and edate='" + exdate + "' and ses_sion='" + sesson + "'");//and degree_code='" + degcode + "' 
                                        }
                                        else
                                        {
                                        ...
                                        else
                                        {
                                           ...
                                           bundleno = Convert.ToString(Convert.ToInt32(bundleno) + 1);
                                        }
                                        //if (!string.IsNullOrEmpty(bun))
                                        //    bun = bun + 1;
                                        if (string.IsNullOrEmpty(bunno))
                                            bunno = bun;
                                        else
                                            bunno = bunno + ',' + bun;
                                        AttSpread.Sheets[0].Cells[AttSpread.Sheets[0].RowCount - 1, 6].CellType = txtceltype;
                                        AttSpread.Sheets[0].Cells[AttSpread.Sheets[0].RowCount - 1, 5].Text = bunno;
                                        AttSpread.Sheets[0].Cells[AttSpread.Sheets[0].RowCount - 1, 3].Text = sbjno;
                                        AttSpread.Sheets[0].Cells[AttSpread.Sheets[0].RowCount - 1, 2].Text = dsv.Tables[0].Rows[k]["roomno"].ToString();
                                        AttSpread.Sheets[0].Cells[AttSpread.Sheets[0].RowCount - 1, 2].Note = exdate;
                                        AttSpread.Sheets[0].Cells[AttSpread.Sheets[0].RowCount - 1, 4].Note = sesson;
                                        AttSpread.Sheets[0].Cells[AttSpread.Sheets[0].RowCount - 1, 1].CellType = cheall;
                                        AttSpread.Sheets[0].Cells[AttSpread.Sheets[0].RowCount - 1, 8].Text = notedegree;//dsv.Tables[0].Rows[0]["degree_code"].ToString()
                                        AttSpread.Sheets[0].Cells[AttSpread.Sheets[0].RowCount - 1, 6].Text = dsv.Tables[0].Rows[0]["regno"].ToString();
                                        k = k + nodtubun - 1;
                                        //k = dsv.Tables[0].Rows.Count - 1;
                                        if (k < dsv.Tables[0].Rows.Count)
                                        {
                                            AttSpread.Sheets[0].Cells[AttSpread.Sheets[0].RowCount - 1, 7].Text = dsv.Tables[0].Rows[dsv.Tables[0].Rows.Count - 1]["regno"].ToString();
                                            ...
                                            //string sv = "update exam_seating set bundle_no='" + bun + "' where regno in ( '" + ksregno + "') and subject_no='" + subjnoNew + "' and edate='" + exdate + "' and ses_sion='" + sesson + "' and roomno='" + dsv.Tables[0].Rows[k]["roomno"].ToString() + "' and degree_code='" + dsv.Tables[0].Rows[k]["degree_code"].ToString() + "'";
                                            string sv = "update exam_seating set bundle_no='" + bun + "' where regno in ( '" + ksregno + "')  and edate='" + exdate + "' and ses_sion='" + sesson + "' and roomno='" + dsv.Tables[0].Rows[k]["roomno"].ToString() + "' ";
                                        ...
                                        else
                                        {
                                            ...
                                            string sv = "update exam_seating set bundle_no='" + bun + "' where regno in ('" + ksregno + "')  and edate='" + exdate + "' and ses_sion='" + sesson + "' and roomno='" + AttSpread.Sheets[0].Cells[AttSpread.Sheets[0].RowCount - 1, 2].Text + "' and subject_no in (select subject_no from subject where subject_code='" + sbjno + "')";//and degree_code='" + AttSpread.Sheets[0].Cells[AttSpread.Sheets[0].RowCount - 1, 8].Text + "' and subject_no='" + subjnoNew + "'
                                            int k1 = d2.update_method_wo_parameter(sv, "text");
            ...
            if (sml.Trim() != "" && sml.Trim() != "0")
            {
                ...
                if (ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    if (ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
                    {
                        ...
                        string overall = "select distinct es.roomno,COUNT(1) as strength,es.ses_sion,es.edate  from registration r,subjectchooser sc,exam_seating as es,Class_master cs  where sc.roll_no=r.roll_no  and exam_flag<>'Debar' and es.regno=r.Reg_No and es.subject_no=sc.subject_no  and cs.rno=es.roomno and cs.Mode='"+ddltype.SelectedItem.Text.ToString()+"' and es.edate='" + datess + "' " + sessiond1 + " group by es.roomno,es.ses_sion,es.edate  ";
                        ...
                        {
                            isval = Convert.ToInt16(AttSpread.Sheets[0].Cells[sew, 1].Value);
                            u = u + 1;
                            if (isval == 1 && u > 1)
                            {
                                int we = 1;
                                ...
                                if (dsoverall.Tables.Count > 0 && dsoverall.Tables[0].Rows.Count > 0)
                                {
                                    ...
                                    mypdfpage.Add(ptc);
                                    string roomnoco = dsoverall.Tables[0].Rows[sew - 1]["roomno"].ToString();

                                    we = we + 1;
                                    coltop = coltop + 35;
                                    ptc = new PdfTextArea(Fontnormal, System.Drawing.Color.Black,
                                                                          new PdfArea(mydocument, 20, coltop, 600, 50), System.Drawing.ContentAlignment.TopLeft, "Date of Exam /Session");
                                    mypdfpage.Add(ptc);
                                    ptc = new PdfTextArea(Fontnormal, System.Drawing.Color.Black,
                                                                          new PdfArea(mydocument, 130, coltop, 180, 50), System.Drawing.ContentAlignment.TopLeft, ":");
                                    mypdfpage.Add(ptc);
                                    ptc = new PdfTextArea(Fontnormal, System.Drawing.Color.Black,
                                                                          new PdfArea(mydocument, 140, coltop, 180, 100), System.Drawing.ContentAlignment.TopLeft, ddlfrmdate.SelectedItem.Text + "/" + ddlsession.SelectedItem.Text);
                                    mypdfpage.Add(ptc);
                                    ptc = new PdfTextArea(Fontnormal, System.Drawing.Color.Black,
                                    ...
                                    mypdfpage.Add(ptc);
                                    ptc = new PdfTextArea(Fontnormal, System.Drawing.Color.Black,
                                                                          new PdfArea(mydocument, 470, coltop, 600, 50), System.Drawing.ContentAlignment.TopLeft, dsoverall.Tables[0].Rows[sew - 1]["roomno"].ToString());
                                    mypdfpage.Add(ptc);
                                    coltop = coltop + 25;
                                    ...
                                    if (dschecksubjlist.Tables.Count > 0 && dschecksubjlist.Tables[0].Rows.Count > 0)
                                    {
                                        //string sml = d2.GetFunction("select value from COE_Master_Settings where settings='Bundle Per Student'");
                                        if (Convert.ToInt32(sml) > 35)
                                        {
                                            sml = "35";
                                        }
                                        int ct = dschecksubjlist.Tables[0].Rows.Count + 1;
                                        if (ct > 35)
                                        {
                                            ct = Convert.ToInt32(sml);
                                        }
                                        Gios.Pdf.PdfTable table1 = mydocument.NewTable(Fontbold, ct, 6, 4);
                                        ...
                                        int gwe = 1;
                                        int gwe1 = 0;
                                        tablerowscount = dschecksubjlist.Tables[0].Rows.Count;
                                        ...
                                        for (int subjlist = 0; subjlist < dschecksubjlist.Tables[0].Rows.Count; subjlist++)
                                        {
                                            ...
                                            if (ast != 0 && ast % Convert.ToInt32(sml) == 0)
                                            {
                                                ...
                                                tablerowscount = tablerowscount - Convert.ToInt32( sml)+1;
                                                mypdfpage.SaveToDocument();
                                                ...
                                                mypdfpage.Add(ptc);
                                                ptc = new PdfTextArea(Fontnormal, System.Drawing.Color.Black,
                                                                                      new PdfArea(mydocument, 470, coltop, 600, 50), System.Drawing.ContentAlignment.TopLeft, roomnoco);
                                                mypdfpage.Add(ptc);

                                                if (tablerowscount > Convert.ToInt32(sml))
                                                {
                                                    tblrocc = Convert.ToInt32(sml)-1;
                                                }
                                                else
                                                {
                                                    tblrocc = tablerowscount;
                                                }
                                                table1 = mydocument.NewTable(Fontbold, tblrocc + 1, 6, 4);
                                                table1.SetBorders(Color.Black, 1, BorderType.CompleteGrid);
                                                table1.Cell(0, 0).SetContentAlignment(ContentAlignment.MiddleCenter);
                                                table1.Cell(0, 0).SetContent("S.No");
                                                table1.Cell(0, 0).SetContentAlignment(ContentAlignment.MiddleCenter);
                                            ...
                                            string regno = dschecksubjlist.Tables[0].Rows[subjlist]["Reg_No"].ToString();
                                            string name = dschecksubjlist.Tables[0].Rows[subjlist]["Stud_Name"].ToString();
                                            string sub_name = dschecksubjlist.Tables[0].Rows[subjlist]["subject_code"].ToString();
                                            string seat = dschecksubjlist.Tables[0].Rows[subjlist]["seat_no"].ToString();
                                            string hallno = dschecksubjlist.Tables[0].Rows[subjlist]["roomno"].ToString();
                                            table1.Cell(gwe, 0).SetContentAlignment(ContentAlignment.MiddleCenter);
                                            table1.Cell(gwe, 0).SetContent(gwe1.ToString());
                                            ...
                                            table1.Cell(gwe, 3).SetContentAlignment(ContentAlignment.MiddleLeft);
                                            table1.Cell(gwe, 3).SetContent(sub_name.ToString());
                                            gwe = gwe + 1;
                            ...
                            for (int sew = 0; sew < AttSpread.Sheets[0].Rows.Count; sew++)
                            {
                                ...
                                if (isval == 1 && u > 1)
                                {
                                    ...
                                    if (dsoverall.Tables.Count > 0 && dsoverall.Tables[0].Rows.Count > 0)
                                    {
                                        ...
                                        //string roomnoco = AttSpread.Sheets[0].Cells[u - 1, 2].Text.ToString();
                                        string queryreg = string.Empty;
                                        queryreg = "select distinct  sub.subject_no,r.Reg_No,r.Current_Semester,r.Stud_Name,r.Stud_Type,es.seat_no,es.roomno,(c.Course_Name +'-'+ d.Acronym) as Grade,(sub.subject_code +'-'+ sub.subject_name) as subjectname,sc.semester,sub.subject_code,sub.subject_name ,c.Course_Name, (select dept_name from Department where d.dept_code=Dept_Code) as deptname,r.degree_code  from registration r,subjectchooser sc,exam_seating as es ,Degree d,course c,Department de,subject sub where sc.subject_no=sub.subject_no and sc.roll_no=r.roll_no  and delflag=0 and exam_flag<>'Debar' and es.regno=r.Reg_No and d.Course_Id=c.Course_Id and d.Dept_Code=de.Dept_Code and d.Degree_Code=es.degree_code and r.degree_code=d.Degree_Code and es.subject_no=sc.subject_no and es.roomno='" + roomnoco + "' and es.edate='" + datess + "'  and es.ses_sion like '" + ddlsession.SelectedItem.Text + "%'  order by r.Reg_No,es.seat_no";
                                        //  queryreg = "select distinct  top 102 r.Reg_No,r.Current_Semester,r.Stud_Name,r.Stud_Type,es.seat_no,es.roomno,(c.Course_Name +'-'+ d.Acronym) as Grade,(sub.subject_code +'-'+ sub.subject_name) as subjectname,sub.subject_code,sub.subject_name   from registration r,subjectchooser sc,exam_seating as es ,Degree d,course c,Department de,subject sub where sc.subject_no=sub.subject_no and sc.roll_no=r.roll_no  and delflag=0 and exam_flag<>'Debar' and es.regno=r.Reg_No and d.Course_Id=c.Course_Id and d.Dept_Code=de.Dept_Code and d.Degree_Code=es.degree_code and r.degree_code=d.Degree_Code and es.subject_no=sc.subject_no and es.roomno='" + roomnoco + "'   and es.ses_sion='" + ddlsession.SelectedItem.Text + "'  order by es.seat_no";
                                        //queryreg = "select distinct  r.Reg_No,r.Current_Semester,r.Stud_Name,r.Stud_Type,es.seat_no,es.roomno,(c.Course_Name +'-'+ d.Acronym) as Grade,(sub.subject_code +'-'+ sub.subject_name) as subjectname,sub.subject_code,sub.subject_name   from registration r,subjectchooser sc,exam_seating as es ,Degree d,course c,Department de,subject sub where sc.subject_no=sub.subject_no and sc.roll_no=r.roll_no  and delflag=0 and exam_flag<>'Debar' and es.regno=r.Reg_No and d.Course_Id=c.Course_Id and d.Dept_Code=de.Dept_Code and d.Degree_Code=es.degree_code and r.degree_code=d.Degree_Code and es.subject_no=sc.subject_no and es.roomno='" + roomnoco + "' and es.edate='" + datess + "' and es.ses_sion='" + ddlsession.SelectedItem.Text + "'  order by es.seat_no";
                                        DataSet dschecksubjlist = new DataSet();
                                        dschecksubjlist = d2.select_method_wo_parameter(queryreg, "text");
                                        if (dschecksubjlist.Tables.Count > 0 && dschecksubjlist.Tables[0].Rows.Count > 0)
                                        {
                                            ...
                                            int gwe = 1;
                                            int gwe1 = 0;
                                            tablerowscount = dschecksubjlist.Tables[0].Rows.Count+1;
                                            //dscheck = d2.select_method_wo_parameter(queryreg, "text");
                                            for (int subjlist = 0; subjlist < dschecksubjlist.Tables[0].Rows.Count; subjlist++)
                                            {
                    ...
                    if (ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
                    {
                        ...
                        for (mm = 0; mm < AttSpread.Sheets[0].Rows.Count; mm++)
                        {
                            ...
                            if (isval == 1 && u > 1)
                            {
                                ...
                                string[] bunbleNew = bndlee.Split(',');
                                for (int i = 0; i < bunbleNew.Length; i++)
                                {
                                    string qrybundleno = Convert.ToString(bunbleNew[i]);
                                y = y + 1;
                                chkgenflag = true;
                                lblerr1.Visible = false;
    ...
    public void loadSubject()
    {
        try
        {
            ...
            {
                SelectQ = "select distinct s.subject_code,s.subject_name from exmtt e,exmtt_det et,subject s where  et.exam_date='" + examdate + "' and e.exam_Month='" + ddlMonth.SelectedValue + "' and e.Exam_Year='" + ddlYear.SelectedItem + "' " + sessiond + " and e.exam_code=et.exam_code and et.subject_no=s.subject_no order by s.subject_name,s.subject_code";
    ...
    public void loadStudent()
    {
        try
        {
            ...
            {
                SelectQ = "select ea.roll_no,r.Reg_No,r.Stud_Name,r.Stud_Type,s.subject_code,s.subject_name,(c.Course_Name+'-'+d.Acronym) as Cource,d.Degree_Code,de.Dept_Name,et.exam_date,et.exam_session,d.Acronym,r.Batch_Year from Exam_Details e,exam_application ea,exam_appl_details ed,subject s,exmtt_det et, Registration r,Degree d,course c,Department de where e.exam_code=ea.exam_code and ea.appl_no=ed.appl_no and ed.subject_no=s.subject_no and s.subject_no=et.subject_no and ea.roll_no =r.Roll_No and r.degree_code=d.Degree_Code and d.Dept_Code=de.Dept_Code and d.Course_Id=c.Course_Id and e.Exam_Month='" + ddlMonth.SelectedValue.ToString() + "' and e.Exam_year='" + ddlYear.SelectedValue.ToString() + "' and  et.exam_date='" + examdate + "' " + sessiond + " order by (c.Course_Name+'-'+de.Dept_Name),ea.roll_no,r.Stud_Type";
    ...
    public void printSubwise()
    {
        try
        {
            ...
            //string hroll = Hasroll[mm].ToString();
            //string bndlee = boundvl[mm].ToString();
            Gios.Pdf.PdfTable table1forpage1 = mydoc.NewTable(Fontco10a, 3, 5, 2);
            table1forpage1.VisibleHeaders = false;
            table1forpage1.SetBorders(Color.Black, 1, BorderType.None);
            table1forpage1.SetColumnsWidth(new int[] { 55, 10, 88 });
            table1forpage1.Columns[0].SetContentAlignment(ContentAlignment.MiddleLeft);
            ...
            table1forpage1.Cell(1, 3).SetContent("                        ");
            table1forpage1.Cell(2, 3).SetContent("                      ");
            Gios.Pdf.PdfTablePage newpdftabpage2 = table1forpage1.CreateTablePage(new Gios.Pdf.PdfArea(mydoc, 25, 115, 550, 550));
            mypdfpage.Add(newpdftabpage2);
            table1forpage1 = mydoc.NewTable(Fontco10a, 3, 5, 1);
            table1forpage1.VisibleHeaders = false;
            table1forpage1.SetBorders(Color.Black, 1, BorderType.None);
            table1forpage1.SetColumnsWidth(new int[] { 65, 10, 88 });
            table1forpage1.Columns[0].SetContentAlignment(ContentAlignment.MiddleLeft);
            ...
            int tblcount = 0;
            if (Convert.ToInt32(Subjectspread.Sheets[0].RowCount - 3) > 17)
                tblcount = 17;
            else
                tblcount = Convert.ToInt32(Subjectspread.Sheets[0].RowCount - 3) + 1;
            table1forpage1 = mydoc.NewTable(Fontco10a, tblcount, 5, 1);
            table1forpage1.VisibleHeaders = false;
            ...
            //newpdftabpage2 = table1forpage1.CreateTablePage(new Gios.Pdf.PdfArea(mydoc, 77, coltop, 450, 400));
            //mypdfpage.Add(newpdftabpage2);
            //tblheight = (int)newpdftabpage2.Area.Height;
            ...
            //mypdfpage.Add(newpdftabpage2);
            ////}
            //printcheckvalue.Text = Convert.ToString(Convert.ToInt32(Subjectspread.Sheets[0].RowCount - 3));
            //int rowscount1 = Convert.ToInt32(printcheckvalue.Text);
            //rowscount1 = rowscount1 + 2;
            //table1forpage1 = mydoc.NewTable(Fontco10a, rowscount1, 5, 1);
            //table1forpage1.VisibleHeaders = false;
            ...
            //    table1forpage1.Cell(i + 1, 0).SetContent(Subjectspread.Sheets[0].Cells[i, 0].Text.ToString());
            //    table1forpage1.Cell(i + 1, 1).SetContent(Subjectspread.Sheets[0].Cells[i, 2].Text.ToString());
            //    table1forpage1.Cell(i + 1, 2).SetContent(Subjectspread.Sheets[0].Cells[i, 3].Text.ToString());
            //    table1forpage1.Cell(i + 1, 3).SetContent(Subjectspread.Sheets[0].Cells[i, 8].Text.ToString());
            //    table1forpage1.Cell(i + 1, 4).SetContent("                                                                                                 ");
            //}
            ...
            mypdfpage.Add(pr7);
            newpdftabpage2 = table1forpage1.CreateTablePage(new Gios.Pdf.PdfArea(mydoc, 22, 70, 550, 550));
            mypdfpage.Add(newpdftabpage2);
            table1forpage1 = mydoc.NewTable(Fontco10, 1, 1, 1);
            table1forpage1.VisibleHeaders = false;
            table1forpage1.SetBorders(Color.Black, 1, BorderType.None);
            ...
            newpdftabpage2 = table1forpage1.CreateTablePage(new Gios.Pdf.PdfArea(mydoc, 25, 735, 500, 200));
            mypdfpage.Add(newpdftabpage2);
            pa1 = new PdfArea(mydoc, 20, 720, 350, 110);//110
            pr3 = new PdfRectangle(mydoc, pa1, Color.Black);
            mypdfpage.Add(pr3);
            table1forpage1 = mydoc.NewTable(Fontco10a, 2, 3, 1);
            table1forpage1.VisibleHeaders = false;
                        ...
                        for (mm = 0; mm < AttSpread.Sheets[0].Rows.Count; mm++)
                        {
                            ...
                            if (isval == 1 && u > 1)
                            {
                                ...
                                for (int i = 0; i < bunbleNew.Length; i++)
                                {
                                    ...
                                    Gios.Pdf.PdfTable table1 = mydocument.NewTable(Fontbold, dsdisplay.Tables[0].Rows.Count + 1, 4, tablepadding);
                                    table1.VisibleHeaders = false;
                                    table1.SetBorders(Color.Black, 1, BorderType.CompleteGrid);
                                    table1.Cell(0, 0).SetContentAlignment(ContentAlignment.MiddleCenter);
                                    table1.Cell(0, 0).SetContent("S.No");
