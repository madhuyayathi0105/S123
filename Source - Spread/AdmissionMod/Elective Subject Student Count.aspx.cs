    ...
    protected void btnGo_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (!string.IsNullOrEmpty(collegeCode) && !string.IsNullOrEmpty(batchYear) && !string.IsNullOrEmpty(qryCourse) && !string.IsNullOrEmpty(collegeCode))
            {
                ...
                qry += " select e.sectionName,e.subjectNo,e.staffApplId,e.staffCode,e.studentCount,sy.degree_code from electiveSubjectDetails e,syllabus_master sy,subject s where sy.syll_code =s.syll_code and s.subject_no =e.subjectNo and sy.degree_code in (" + Degree + ") and sy.Batch_Year in (" + ddlBatch.SelectedValue + ") and sy.semester ='" + ddlSem.SelectedValue + "' " + CheckValue + "";
                dsCourseDet.Clear();
            ...
            if (cbSectionWise.Checked == false)
            {
                if (gvSectionWiseCount.Rows.Count > 0)
                {
                    foreach (GridViewRow gvSecRow in gvSectionWiseCount.Rows)
                    {
                        ...
                        {
                            qry = "if exists (select * from electiveSubjectDetails where subjectNo='" + lblSubjectNo.Text.Trim() + "' and staffApplId='" + lblStaffApplId.Text.Trim() + "' and staffCode='" + lblStaffCode.Text.Trim() + "' and isnull(issectionwise,'0')=0) update electiveSubjectDetails set studentCount='" + studCount + "' where subjectNo='" + lblSubjectNo.Text.Trim() + "' and staffApplId='" + lblStaffApplId.Text.Trim() + "' and staffCode='" + lblStaffCode.Text.Trim() + "' else insert into electiveSubjectDetails (subjectNo,staffApplId,staffCode,studentCount,issectionwise) values('" + lblSubjectNo.Text.Trim() + "','" + lblStaffApplId.Text.Trim() + "','" + lblStaffCode.Text.Trim() + "','" + studCount + "',0)";
            ...
            if (cbSectionWise.Checked == true)
            {
                if (DivgvSectionWiseCount.Rows.Count > 0)
                {
                    foreach (GridViewRow gvSecRow in DivgvSectionWiseCount.Rows)
                    {
                        ...
                        if (!string.IsNullOrEmpty(lblSubjectNo.Text.Trim()) && studCount > 0 && !string.IsNullOrEmpty(lblStaffApplId.Text.Trim()) && !string.IsNullOrEmpty(lblStaffCode.Text.Trim()))
                        {
                            qry = "if exists (select * from electiveSubjectDetails where subjectNo='" + lblSubjectNo.Text.Trim() + "' and staffApplId='" + lblStaffApplId.Text.Trim() + "' and staffCode='" + lblStaffCode.Text.Trim() + "' and SectionName='" + lblsection.Text + "') update electiveSubjectDetails set studentCount='" + studCount + "' where subjectNo='" + lblSubjectNo.Text.Trim() + "' and staffApplId='" + lblStaffApplId.Text.Trim() + "' and staffCode='" + lblStaffCode.Text.Trim() + "' and SectionName='" + lblsection.Text + "' else insert into electiveSubjectDetails (subjectNo,staffApplId,staffCode,studentCount,SectionName,IsSectionWise) values('" + lblSubjectNo.Text.Trim() + "','" + lblStaffApplId.Text.Trim() + "','" + lblStaffCode.Text.Trim() + "','" + studCount + "','" + lblsection.Text + "',1)";
