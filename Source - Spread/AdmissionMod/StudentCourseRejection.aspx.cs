...
public partial class AdmissionMod_StudentCourseRejection : System.Web.UI.Page
{
    ...
    public void bindBranch()
    {
        try
        {
            ...
            DataSet dsBran = d2.select_method_wo_parameter("select d.Degree_Code,dt.dept_name from Degree d, Department dt,course c where dt.Dept_Code=d.Dept_Code and c.Course_Id=d.Course_Id and d.college_code='" + ddlCollege.SelectedValue + "' and c.Edu_Level='" + ddlEduLev.SelectedValue + "' and d.Course_Id='" + ddlcourse.SelectedValue + "' and d.degree_code in (select Degree_Code Remaining from seattype_cat where collegeCode='" + ddlCollege.SelectedValue + "' and Batch_Year='" + ddlbatch.SelectedValue + "' and Quota='" + ddlCategory.SelectedValue + "' and Category_Code='" + streamValue + "' and (ISNULL(Tot_Seat,0)-allotedSeats)>0) order by Dept_Name asc ", "Text");
                ...
                if (ds.Tables != null)
                {
                    ...
                    if (ds.Tables.Count > 2 && ds.Tables[1].Rows.Count > 0)
                    {
                        ...
                        if (ds.Tables[2].Rows.Count > 0)
                        {
                            ...
                            DataTable dtRegDet = dirAcc.selectDataTable("select a.quota,r.degree_code,dt.dept_name from registration r,applyn a,degree d,department dt where a.app_no=r.app_no and d.degree_code=r.degree_code and dt.dept_code=d.dept_code and a.app_formno='" + txt_applicationno.Text + "' and a.college_code='" + ddlCollege.SelectedValue + "'");
    ...
    protected void btnReject_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                dirAcc.deleteData("delete from registration where app_no='" + appNo + "'");
                dirAcc.deleteData("delete from ft_feeallot where app_no='" + appNo + "'");
                ...
                clearDetails(false);
                ScriptManager.RegisterStartupScript(this, typeof(Page), UniqueID, "alert('Rejected successfully')", true);
